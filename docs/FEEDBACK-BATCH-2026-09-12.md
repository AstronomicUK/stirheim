...
Latest broad evidence: **2,402 ordinary tests across 200 files**, **304 API tests across 54 files**, TypeScript and production build pass after the Cavalcade integration through 115. Six additional focused Slaaneshi roller/probability tests pass. The flag is now enabled locally and the actual mobile attack → held model → end confirmation → exact captive/report flow passes. Tabletop entry, companion targets and further regression remain under work. The Engine rescue card is still a design preview; custody and journeys are connected and browser-verified. Nothing in this batch has been deployed.
