...
Tom is now awake and has authorized continuing this batch to deployment readiness. The overnight heartbeat is paused. Reload-house-rule allocation, specialist agreed-ruling handling, and Hired Sword Awakening eligibility have been raised for his answer. No new deployment authorization is implied by this instruction.

Tom subsequently requested one full Claude handover, then no further Claude work to preserve his weekly usage. Request `ce20c629` was queued on the shared bus: existing knowledge only, commits/uncommitted files, outstanding work, actual verification, risks and process ownership; no extra research or tests. Receipt/reply is not yet confirmed. Codex owns the remaining batch; do not send further work or periodic wake requests to Claude without Tom authorizing resumption.

**Handover received:** `d5e89bf1` and rescue review `76844531` read and acknowledged. Claude stopped, released claims and killed his listener; no further assignments. His last implementation is `e9e3c6f` (107); his last full run was 295 API tests, superseded by Codex's later 322 run. His mention of working changes in `captives.ts`/`engineCustody.ts` was stale: both are currently clean. No blocking rescue defect found by his source review, but V1 rescued group UI availability, V2 unknown-key authority and V3 physical casualty indices remain verification notes. New Sacrificial Ritual omission is recorded under #229. Further risks: `warband_leader_unit()` only maps black_dwarfs; generated trait/rate seeds need regeneration if templates change; completed journeys are final except manual GM correction. Codex owns these follow-ups. Engine destruction inventory consequences remain separate from freeing its captives.

**Handover follow-through:** Migration 128 now generates the leader catalogue from the actual roster templates and mirrors native/permanent/temporary succession plus Dreamer-before-Priest precedence. A transactional database regression also exposed direct API-role permissions on the supposedly private `warband_leader` helper; 128 explicitly revokes anon/authenticated access. Catalogue parity, succession, hired exclusion, Dreamwalker fallback and permissions pass; local 128 applied, Engine journey 4/4 and rescue 8/8 checks pass. TypeScript and changed-file lint pass. V1 has no group-cap UI block; V2 deliberately requires the Engine owner/GM when no new key holder is known, now explained precisely in the form and tested; V3 manual casualty ordinals do not identify physical models, now documented and regression-tested (3 prompt tests pass). Nothing deployed.

**Further checkpoint:** Full API run through 128 passes **323/323 in 56 files**; all **122 migrations** through 128 replay in isolation. Follow-up 129 also supports temporary leaders for the two catalogue warbands with no mandatory leader type; transactional succession/permission checks pass and 129 is applied locally. Ritual foundations now validate selection and affected players and apply extra-sacrifice Difficulty reductions correctly (38 casting/selection tests pass); they do not yet consume captives or expose a finished spell flow. See `SACRIFICIAL-RITUAL-INTEGRATION-2026-09-13.md` for remaining integration and the pending first-captive timing ruling. No deployment.
...
| #229 further covered flows | Pirates; Subjugator Hero/group/companion capture; ordinary Man-catcher; Engine custody, exploration and journeys; Cavalcade combat/manual capture and Throne; Slaaneshi per-model holds, companions and correction history; Engine rescue keys/contact/escape and agreed return | Final cross-flow review; source-ambiguous rescue aftermath; Cavalcade reward overflow ruling; Ogre Gluttony; Pit Fighters’ In the Pit |
...
Tom says to keep working until he messages that he is awake. Claude completed custody 102/104 and journeys 105/107; Codex integrated the tested UI in 0366e3d and 5526952 and added exploration 106. Local-only follow-ups 108 preserve reward-recipient names and 109 protect the between-battles boundary. Migration 110 fixes actual transition timestamps after a delayed battle start. Migration 111 enforces the five-Thrall reward limit at the server. Migration 112 adds henchman Throne outcomes; 113 connects actual combat/manual captures and 114 corrects human eligibility. Migration 115 adds verified Hero 61 capacity rerolls. Migrations 116–119 connect verified Slaaneshi holds, companions, reports and repeated-hit corrections. Migrations 120/121 connect Engine rescue battle facts and post-battle returns; 122 adds ordered corrections and anonymous departure protection. Migration 123 links combat casualties to prison-key prompts; 124 adds Court henchman conversion. The next unreserved migration number is 128. Claude has been sent the rescue-contract review, but has not yet acknowledged that next assignment; Codex is continuing independently. Do not reset the shared local database, push individual commits, or publish a partial release.
...
## Post-111 release checks

Production build passes (existing marker CSS/large-chunk warnings). All **105 migration files** replay successfully in filename order in an isolated fresh database, which is then removed. The serial API run passed 295 of 296 checks; its one failure was a test-order assumption: simultaneous feedback submissions can allocate IDs in a different order from the input array, but the test sorted actual notifications by URL while leaving expected URLs unsorted. Corrected the expected sort and selected the title assertion by issue identity; no application notification change required. This is separate from the earlier all-green 295-test checkpoint.

## Cavalcade henchman outcome integration — local 112

The actual attack roller now has a tested Capture! step foundation: a qualifying actual Misericordia OOA triggers its D6 after injury, records app/tabletop origin, preserves the ordinary casualty on failure, and never rolls another planned attack. 79 roll-through tests pass. The eligibility flag is not yet supplied by live FightTab; the source-event and manual paths remain to be connected.

Migration 112 and buildHenchmanThrone resolve the saved henchman case without removing another model: 1–2 removes the captive permanently, 3–5 grants one printed zero-XP Captured Thrall (five-model cap, exempt from ordinary roster size), 6 grants exactly one XP and any actually crossed advancement to the selected surviving ordinary Hero. All outcomes transfer only the case’s exact annotated equipment. The server rejects ordinary ransom/sale/exchange for Cavalcade Hero/henchman cases. The roster UI now routes a saved Cavalcade henchman case to a dedicated Throne form; visual browser acceptance is next.

12 forced-capture API tests pass, including all three new outcome branches through separate players’ consent and GM reversal, missing kit, impossible original dice, forbidden second model removal, missing advancement, no premature award, no double acceptance, exact kit and retained original/edited dice. These fixtures isolate the outcome contract using the established forced-capture report fixture; they do not claim a live Misericordia trigger yet. TypeScript passes. Nothing deployed.
...
### 13 September — Hero capture limits (115)

A Hero Captured result now asks for another D66 if the identified Cavalcade captor already has five Thralls or two captures. Multiple Heroes in one pending report share the same limit. The rejected result, original app/tabletop provenance and explanation persist when the next die is recorded, so a later capacity change cannot turn it back into a capture. District recovery rules retain precedence. The server rechecks new cases and captor assignments under the captor lock; a rejected report leaves the roster untouched.

Validation: 2,402 ordinary tests pass; build and TypeScript pass. Four local captive-dice API checks cover rejection at five Thralls, rejection of three Heroes in one report, acceptance of two, and existing consent outcomes. Mobile saved-draft UI replay passes original app 61 → required reroll → 41 → reload after capacity changes; no overflow or browser errors. Test-fixture step-index and locator errors were corrected before the successful run. Full API regression passes: 304 tests across 54 files. Multiplayer cases whose captor is still unknown retain the existing assignment flow; a full Cavalcade assignment is rejected and requires correction of the source report.

### Slaaneshi live integration — local 116/117, not deployed

Mobile actual-combat verification passes: model 2 held, no OOA/kill XP, end confirmation, no injury dice, exact captive and two axes removed from a four-model/eight-axe group. Explicit releases and subsequent Recovery preserve per-model conditions. Weapon switching asks for release before beginning the next attack; tabletop capture entry is being verified. Companion targets and further regression remain open.

### Checkpoint 6705d90 and companion extension 118

116/117 checkpoint: 2,414 ordinary tests, 311 API tests, TypeScript/build and all 111 then-current migration files passed. Mobile roller, tabletop and weapon-switch flows passed. Companion extension 118 now passes actual roller and tabletop mobile flows for wardog 2 of 2, preserving its notes and leaving one with the handler. Eight hold API checks and five focused report/companion checks pass, with all 112 migration files installing in isolation. No deployment. Further active-hold attack handling remains under review.
...
### Engine rescue returns — local 121

Named escaped prisoners return only after the rescue match is completed, through an ordinary two-player proposal. Hero/Hired Sword status is restored without kit; a henchman returns as one separate original-profile model, never merged into an incompatible group. Anonymous escape frees custody after owner/GM confirmation. Accepted outcomes support GM reversal; named return reversal restores Engine custody and preserves confiscated items. Freed/escaped prisoners cannot be selected for dispatch while their return is pending. Mobile proposal from the completed match page → separate player acceptance → exact kit check → GM reversal passes. Nine targeted Engine API checks and all 115 migrations pass. Mistaken battle-fact corrections and anonymous departure reversal are next in 122. Nothing deployed.
...
### Key-holder loss notifications — local 125

The controlling player receives one private notification when their particular key holder is taken out of action, even when keys cover several Engines. A different member of the same group does not trigger it. Reversing the casualty retracts the notification. Five Engine API checks and all 119 migrations pass. No deployment.


### Rescue input validation — local 126

Missing reviewed revisions now reject actions and corrections rather than bypassing the stale-state check. A defeated Gaoler cannot be assigned their own keys; an unknown keeper remains an explicit valid choice. Six Engine API checks, seven rule/prompt checks and all 120 migration files pass. No deployment.
