...
**Status:** 🟡 Core firearm reload and pistol/brace close-combat actions verified locally; supplementary weapons and model-count clauses remain. Not deployed.
**Priority:** 🟡 Low
...
### #68 Merchant Bribery — cost calculation in progress, 11 September

A tested quote now identifies a participating, surviving Hero with the learned Bribery skill and counts remaining non-Hero members, including Hired Swords and purchased animal fighters. Casualties and absentees are excluded from payment; Heroes are not charged. It calculates 5 gc per remaining non-Hero, affordability, the remaining Rout count after prior paid exclusions, and whether a test would still be required after another payment. Source1c:2298 checked. Fourteen Rout tests plus typecheck/build/lint pass with existing warnings. **No payment control is exposed yet:** atomic treasury deduction, durable receipts/exclusions, shared totals and logs still require integration. No deployment.

### #68 Merchant Bribery — paid flow fixed locally, 11 September

The Rout warning now offers Bribery for a surviving Hero with the learned skill. It shows the remaining non-Hero breakdown and asks the player to confirm that count and the Merchant’s ability to act, then pays 5 gc per remaining member. Gold deduction and an immutable payment receipt are one transaction; retries cannot charge twice, competing stale quotes are rejected, and insufficient funds or missing permissions/skill cannot debit the treasury. Each receipt excludes one casualty from Rout tests for the rest of the battle, while actual casualties and injury tallies remain unchanged. Additional payments are available only while a test is still required. Counts include remaining Hired Swords and animal fighters.

Paid exclusions survive reloads and update both players’ Rout summaries. Payments appear in the shared battle log in both app-calculated and player-calculated battles, and the treasury audit has an English explanation. The no-hirelings/zero-cost edge is left as an explicit table exception, not an unlimited free payment. The server treats the shown physical model count as the player’s confirmed declaration; it independently validates identity, participation, skill, funds and payment history.

Validation: **1,782 ordinary tests and all 173 local database tests pass**; 52 focused battle tests, build and lint pass with existing audit-file warnings. Disposable mobile QA in both combat modes verified one debit, the reduced Rout requirement, unchanged casualties, reload persistence and readable receipts. Migration **73** is local only and must accompany the combined release. During local setup, out-of-sync migration history replayed the old migration-50 Fanatic function before stopping on an existing migration-51 column. Restoring the already-current migration-61 function resolved the resulting sole regression; the final complete database run passed. No production data or deployment changed. #68 remains partial for Merchant Wagon abandonment and other outstanding Rout mechanics.

### #111 Flesh Construct survival experience — fixed locally, 11 September
...
## 2026-09-11 Wight Blades — #114 partial, combat correction (local)

Standard Restless Dead Grave Guards and promoted Wights now receive automatic wounds on natural melee hits of 6 with their held weapons; the wound roll still checks for a critical. Poison immunity does not cancel this magic. Unpromoted Wights do not receive it. Corrected the shared-name parser that had incorrectly assigned the variant's 5+ critical rule to standard Grave Guards. Variant Grave Guards retain their separate 5+ rule and material restrictions; variant promoted Wights are not given a blade grant absent from their promotion clause. Promoted standard Wights show their granted rule on the roster and battle sheet. Exact identities keep roster and simulator behavior aligned. Source: grade-1c.md:3371,3459; restless-dead-variant.md Wight Blades and Experience. Build/lint pass, 297 engine/battle/simulator tests pass; final typecheck and63 roster/battle tests pass. Three existing audit-script lint warnings remain. #114 stays partial for its other promotion consequences. No deployment or live history changes.


## 2026-09-11 Snotling follow-up — #114 partial / #125 fixed locally

#114: Promoted Runts and Shoota Teams now show the full Mob Rule and Rigors of Leadership rules on roster and battle-sheet cards. Promoted Runts no longer show Teeny Hands; unpromoted Runts retain it. Original Snotling heroes show the full rule rather than a circular reference. Rigors survival XP was already corrected. Nearby-model counting/Leadership bonuses remain table-managed; this does not claim automatic positioning or unpromoted Runts' equipment restrictions. Other special promotion consequences remain outstanding.

#125 fixed locally: Snotling Mobs die only on post-battle injury rolls of 1–2. Shoota Teams retain 1–3 and Runts 1–4. Verified every die result for each distinct unit through the injury resolver; no prior casualties or live histories changed. Source grade-2a-part2.md Not So Small, Surprisingly Tough; promotion source That (Git)'s Got Talent and Teeny Hands. Build/lint and47 focused tests pass for promotion display; final typecheck and261 campaign/post-battle tests pass after the injury fix. Existing three audit-script lint warnings remain. Awaiting combined deployment.


## 2026-09-11 Life of Slavery advancement — #114 partial (local)

Arabian Tomb Raider Slaves rolling Lad's Got Talent now lose one group member to execution, with a clear review before confirmation. Surviving members receive a replacement advance; the last member's removal dissolves the group without creating a hero or follow-up. The result and original dice audit are stored in plain English. Existing transactional advancement saving prevents duplicate confirmation from removing another member or queuing another roll. Source grade-1b-part1.md:698 explicitly requires execution and the remainder's reroll. Ordinary advances remain unchanged. Build/lint, final typecheck, 58 model/roster tests and all4 local advancement/trading database tests pass, including new last-member/remainder/double-submit checks. Existing three audit-script lint warnings remain. Other #114 consequences, including Orc Goblins, remain outstanding; no deployment or live history changes.
...
## 2026-09-11 succession correction — #121 fixed locally

Core succession now ranks highest Leadership first, then Experience, while preserving named warband candidates and explicit Experience-based exceptions. Appointment retains the successor's original skill lists while keeping the existing leader-equipment access. A genuine top-candidate tie has no automatic selection: the screen names the tied heroes, instructs a D6 decision at the table and requires the user to select its result before appointment. Player selection remains available for agreed exceptions. Source 03-campaigns-magic-optional-rules.md:81. Build/typecheck/lint and5 succession tests pass; actual mobile tie/disabled-button/selection/appointment/original-skill-list/reload check passes. Three old audit-script lint warnings remain. #122's species, temporary leadership and warband-specific follow-ups remain separate and incomplete. Local only.

**#114 Slayer mobile verification:** Hurlers and Stubbles passed actual local mobile promotion, mandatory Deathwish grant, consumed immediate advance, no extra Hero roll, remaining-group reroll only, saved app/manual dice audit and reload. Disposable fixtures removed. An initial local JWT clock mismatch cleared on retry; no product change was needed.


## 2026-09-11 promotion remainder — #178 fixed locally

A group's follow-up after promotion now carries a durable database flag. The remaining members must reroll any 10–12, including after reload or dice edits; future independently earned advances may still promote a member. The server derives this flag from the promotion and also refuses a second promotion on the marked advance before changing the roster. The original new hero keeps their immediate roll, except the separately implemented Slayer Deathwish substitution. Source 03-campaigns-magic-optional-rules.md:290. Migration74 applied to the local database only; existing historical unmarked rows are not guessed or rewritten. Build/typecheck/lint and54 focused tests pass; all5 local advancement/trading database tests pass; mobile end-to-end promotion → remainder12 blocked → reload still blocked → reroll2 resolved passes. Three old audit-script lint warnings remain. Pending combined deployment.


## 2026-09-11 optional Hero replacement — #211 implemented locally, mobile check pending
...
### Priority project — #122 Ogre Hunting Party temporary leadership (2026-09-11, local)

Implemented the printed Ideas Above Their Station sequence: an appointed Gnoblar keeps their original unit type, characteristics, experience, skill lists, equipment and species-dependent rules. A persistent temporary-leader flag identifies their role in the roster, battle Leadership/bonuses, pre-battle prompts, leader-only skills and post-battle leader selection. Promoted Gnoblar Fighters and Flingers are eligible alongside Trappers and the Sabre Baiter. The roster displays “Temporary leader”. Recruiting a replacement Ogre Hunter clears the temporary role and makes the new Hunter leader; losing that Hunter requires a fresh succession choice. Starting-warband validation still requires the printed leader.

Source: reference/rules/warbands/grade-2a-part1.md, Ideas Above Their Station (line 2761). Validation: 144 focused tests; build/typecheck and lint passed (three pre-existing warnings in the untracked audit probe). Disposable local mobile QA at 390px confirmed appointment, unchanged Gnoblar identity/stats/skill list, stored role, visible label and reload. Unit tests additionally cover replacement recruitment and a subsequent leader death. No production deployment or historical roster repair. #122 remains partial: other warbands’ inherited skills/spells, purchase delays and temporary/permanent succession rules remain open.


### Priority project — #122 Merchant successor skill access (2026-09-11, local)

A Merchant Caravan successor now inherits access to the Merchant skill table in addition to their original lists. This is the printed exception to the normal succession rule retaining original skill lists; it does not import all of the old leader’s standard tables or grant any learned skill for free. Existing Merchant re-templating already supplies the Merchant identity/rule. Source: reference/rules/warbands/grade-1c.md:2247. Validation: 36 succession/advancement tests and build/typecheck/lint passed (three existing audit-probe warnings). Tests cover the previously missing skill list, available Bribery, retained original skills/lists and duplicate prevention. #122 remains partial, notably apprentice recruitment when no eligible successor exists, other warband inheritance and timing. Saved locally; no deployment.


### Priority project — #122 Battle Monks Decree and temporary succession (2026-09-11, local)

After an Emissary’s recorded death, paid purchases of other warriors or equipment are blocked until a replacement Emissary is hired. The trading sheet explains Decree and disables buying; recruitment lists explain blocked hires. Resolver checks also cover paid Hero/henchman hires and veteran charges. Free acquisitions, sales and transfers remain available. A surviving Hero leads temporarily without becoming an Emissary; candidates use normal Leadership/Experience priority, rather than the previously unsupported automatic Officer preference. Recruiting the replacement clears the temporary role and immediately restores purchases.

Source: reference/rules/warbands/grade-1c.md:175 (Decree), with core succession priority. Validation: 80 focused tests; build/typecheck/lint passed with existing audit-probe warnings. Disposable 390px mobile QA passed temporary appointment, stored original identity, Decree warning and disabled purchase/recruitment, replacement hiring, cleared role and restored equipment buying. Uses existing recorded dead Emissary history, no historical repair or new database migration. Other #122 warband cases remain open. Local only, no deployment.


### Priority project — #69/#156 Whipcrack in the off hand (2026-09-11, local)

Found and fixed a remaining attack-allocation gap: wielding a sword in the primary hand and a whip in the other hand previously lost the Whipcrack bonus. The bonus is now allocated to the first actual whip, whichever hand holds it; two whips still receive only one Whipcrack bonus in total. Frenzy doubles the basic attacks, not the off-hand or Whipcrack additions. Battle odds, sensitivity tables and the shared combat engine use the same allocation. Source: reference/rules/02-weapons-armour-equipment.md:692 (also other whip entries).

Validation: 349 engine, battle and simulator tests; build/typecheck/lint passed (existing audit warnings). Tests cover both hand orders, two whips, charging/first-turn context, later turns, Frenzy and failed Stupidity; battle odds show the extra whip attack and the corresponding combined probability. No browser QA or deployment for this calculation change. The when-charged bonus’s separate Strike First advice/resolution remains part of #156; this does not claim that entire item complete.


### Priority project — #156 separate Whipcrack/Whirlwind strike advice (2026-09-11, local)

Strike-order advice now separates normal attacks from a charged defender’s one Whipcrack attack. It compares that bonus attack with the charge using selected-weapon Initiative, explains a tie/roll-off, respects Strike Last and Strongman, and states the charger-only/one-bonus limit. Whirling Blades use their distinct printed Whirlwind of Death rule: the Dance of Doom and off-hand attacks gain Strike First, while normal attacks retain normal order. Merely carried/unselected whips and later combat rounds do not trigger the advice. For the attacker’s generic first-turn context, wording remains conditional on actually having been charged; it does not invent charge state.

Sources: 02-weapons-armour-equipment.md:692; warbands/grade-2a-part1.md:861–865. Validation: 119 focused tests plus build/typecheck/lint passed (existing audit-probe warnings). No browser QA or deployment. This is strike-order advice, not automatic interleaving of two players’ dice sequences. #156 remains partial for full opposing attack sequencing; the older claim that selected defender weapons remain unfixed is superseded by prior local work.


### Priority project — #152 Sunstaff (Lustria) close-combat profile (2026-09-11, local)
...
### Priority project — #70 persistent recorded Stupidity failures (2026-09-11, local)

For individual warriors, the Battle Sheet’s failed-Stupidity choice is now saved and survives changing panels or reloading. It prevents starting attacks and casting spells until the start of that warrior’s next own turn, including opponents’ turns across a round boundary. The shared turn order determines expiry; legacy sheets use their own turn counter. Clearing the checkbox records a correction, and the history explicitly identifies the player’s declaration rather than inventing an app-rolled Leadership test. Casting leaves no spell attempt when blocked. Deathwish/removed Stupidity traits do not retain the restriction.
...
### Overnight batch — #114 further promotion paths (2026-09-11, local)

Orc Goblin Talent now executes one member instead of silently re-rolling; surviving members receive a new roll and the last member removes the group. Chapel Squires explicitly choose Squire (normal immediate advance) or Knight Errant (Knight/Vain/Impetuous and Special Skills instead); earned stats/XP persist and incompatible equipment returns to the stash. Sorcerous Untrained gain Wizard with a persistent lore and choose their immediate Hero advance or a random spell instead. A Grunt may become a mundane Hero or split off as an Untrained henchman, retaining his earned profile; a later Talent result can make him a Wizard. Lustrian Prospects now have an explicit ordinary-Talent prohibition; WEB Fanatic coverage verified.

Validation: 1,976 ordinary tests pass (176 database tests skipped in ordinary mode); build passes; lint only three existing audit warnings. Dedicated mobile plus local database checks passed for Orc surviving/last-member casualty, both Squire paths, both Untrained paths and both Grunt paths. Confirmation is atomic and queues the correct immediate/group advances; equipment transfer and saved Wizard lore/spell verified. All fixtures disposable. No new SQL, no push or deployment yet. #114 remains partial: Wretch injury and Lustrian replacement inheritance outstanding. #122 still in progress; see OVERNIGHT-RULES-BATCH-2026-09-11.md.


### Overnight batch — #114 Lustrian inheritance / #122 Thrall chain (2026-09-11, local)

Lustrian Prospects can replace a lost/retired Hero via a snapshot-checked transaction. They retain their own stats/XP and two chosen tables, adopt the lost role, inherit its retained equipment and put their old kit in the stash. The vacancy is consumed once across future successor deaths. Historical Hero types cannot be bought again; dismissed Reaver kit is retained for the replacement. Migration75 queues the immediate Hero advance even for a zero-XP Prospect; no extra advance for surviving group members. Captive-kit ownership remains an explicit source conflict for Tom’s morning ruling.
...
## #68 actual mobile filing verified; captured stock excluded from rewards
Mobile390 /tmp/stirheim-wagon-file-mobile-qa.mjs now completes all8wizardsteps and taps File report (not a directsubmitshortcut). Confirmsonependingledger,originalwagon/cargoremoved,merchant80gc/0shards,warbandscreenpendingpanel; publicGMwithdraw restoresoriginalIDs/80gc/3shards. Fixturehasnoheroesandnonewfinds; separateSQL85publicsubmitintegration coversnewexplorationfinds. Noerrors/overflow,cleanupverified.
Moved snapshot derivation beforepostbattlechoices; afterTradeWagonCapture suppliesfiltereditems/emptiedoriginalstash tolocationrecruits,lycanthropechoicecalculationandShrineblessing. Rawctx remainsfororiginalsnapshot/injuryvalidators. No-capturecontextunchanged. Addedtest originalcargoimmutable/carriedkitretained/rewardchoicesexcludeheldstock. Fullordinary2111pass/206DBskipped, productionbuild/typecheck/focusedlintpass. /tmp/stirheim-wagon-complete-flow-{tests,build}.log. NoSQL/push/deploy.
Asked Tom an async rule question this heartbeat: contents-only theft emptywagonfate, choices GMrecordsreturned/lost (first option), alwaysreturn, alwayslost. This is a REQUIRED ruling before implementing that dependentoutcome; elapsedtimeisnotananswer. Do not askagain. Keep/ransom,reportfile/withdraw,cargoguards,raresearchrestrictionandmobilecontrolsnowcovered. #68remainspartialforcontents-onlytheft untilanswered; do notclaimallfixed. Source#68specificallymentionsabandonment+captorrares earchrestriction, notfullwagonmovement/destruction/storageeconomy, so do notexpandintoadditionalvehicleproject.
NEXT ifanswerarrives implementchosenlootflow+checkedundo+UIandtests. Whilewaiting, independentlypreparecore-rulebook-firstremainingprojectpriorityreview (noimplementationofnewprojects). Reviewlatesttrackercheckpoints ratherthanoldmainentry statuses alone. Batchstilllocalonly. Testgroup-formactualreportifnewconcernarises; existinggroupmodel+DBkeep/undo testsalreadycoveridentity.
...
**Approved release split — 13 September:** Ship the locally verified core flows and already completed specialist work with the main release. Unfinished specialist extensions remain a follow-up, including Sacrificial Ritual, Ogre/Pit Fighter outcomes and unresolved Engine/Cavalcade/Court boundaries. Hired Sword Awakening eligibility also remains deferred. Keep #229 working on; see `MAIN-RELEASE-SCOPE-2026-09-13.md`.
...
**Release candidate — 13 September, local only:** Chamber displays, Nuln/Ostlander handling, sequential shooting/melee, player-calculated use/correction and mini View Rosters are verified. All three campaign reload policies are connected. Tom’s screenshot supersedes the earlier intermediate-rule interpretation: eligible weapons can fire 2/1/2/1 barrels on successive own turns without a non-firing phase. Shared chamber checks and pistol selection no longer block that cycle; ordinary reload and misfire restrictions remain. Both mobile battle modes and settings save/refresh pass. Main-release checks: 2,435 application tests, 323 backend tests, production build and all 124 migrations through 130 pass. Ready for deployment; not deployed or marked implemented yet.
