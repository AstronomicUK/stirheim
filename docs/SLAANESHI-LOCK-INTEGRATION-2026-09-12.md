...
Migration 119 preserves one hold when further hits pin the same model. Its supporting attack history survives corrections; reversing one attack cannot erase a remaining valid hit or revive a later agreed release. A new target is blocked before dice while the catcher is occupied. Repeated app/tabletop mobile flow passes through one final captive, and 26 focused API checks pass. All 113 migrations replay in isolation. Equipment companions and repeated hits are no longer integration gaps.
