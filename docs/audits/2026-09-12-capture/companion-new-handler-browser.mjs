...
try {
 ...
 must(await admin.from('heroes').update({status:'retired'}).eq('id',handler.id));
 const replacement=must(await admin.from('heroes').insert({warband_id:ids[1],name:'QA New Handler',unit_type_rules_id:'mercenaries_reikland_champion',status:'active',stats:{M:4,WS:4,BS:3,S:3,T:3,W:1,I:3,A:1,Ld:7}}).select('id').single());
 ...
 await p.getByRole('combobox',{name:'Return companion to',exact:true}).selectOption(replacement.id);
 await p.getByRole('button',{name:'Record agreed outcome',exact:true}).click();
 await expect.poll(async()=>must(await admin.from('items').select('quantity,notes').eq('holder_id',replacement.id).eq('item_rules_id','wardogs'))).toEqual([{quantity:1,notes:'QA companion notes'}]);
 ...
 expect(errors).toEqual([]);
 console.log('PASS: mobile companion return lets the players choose a new active handler when the original handler has left, preserving notes and quantity.');
