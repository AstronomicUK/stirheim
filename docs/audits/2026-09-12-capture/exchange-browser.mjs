...
try {
 ...
 const dog=must(await admin.from('items').insert({warband_id:ids[1],holder_type:'hero',holder_id:handler.id,item_rules_id:'wardogs',quantity:2,notes:'QA companion notes'}).select('id').single());
 ...
 await p.getByRole('button',{name:'Next',exact:true}).click();
 await expect(p.getByText(/Captured by Subjugator of Mankind; no injury roll/)).toBeVisible();
 ...
 must(await player.rpc('submit_battle_report',{p_match_id:match,p_warband_id:ids[1],p_report:{result:'lost',applied:{item_patches:[{id:dog.id,quantity:1}],captured_companions:[{sourceItemId:dog.id,holderId:handler.id,itemId:'wardogs',animalId,eventId:attack.id,captorWarbandId:ids[0],reason:'subjugator'}]}}}));
 must(await player.rpc('submit_battle_report',{p_match_id:match,p_warband_id:ids[0],p_report:{result:'won',injuries:[{subjectType:'hero',subjectId:heroes.find(h=>h.warband_id===ids[0]).id,subjectName:'QA Captain',rolls:[61],outcome:'captured',injuryCode:'captured',injuryName:'Captured',effect:''}],applied:{heroes:[{id:heroes.find(h=>h.warband_id===ids[0]).id,patch:{status:'captured',flags:{captured:true}}}]}}}));
 ...
 await p.getByText('Exchange for another captive',{exact:true}).first().click();
 await p.getByRole('combobox',{name:'Captive returning to the other warband',exact:true}).first().selectOption({index:1});
 await p.getByRole('button',{name:'Record agreed exchange',exact:true}).first().click();
 await expect.poll(async()=>must(await admin.from('items').select('quantity').eq('id',dog.id).single()).quantity).toBe(2);
 expect(must(await admin.from('items').select('notes').eq('id',dog.id).single()).notes).toBe('QA companion notes');
 const resolved=must(await admin.from('captive_cases').select('state,resolution_kind').eq('match_id',match));
 expect(resolved).toHaveLength(2);
 expect(resolved.every(c=>c.state==='resolved'&&c.resolution_kind==='exchange')).toBe(true);
 expect(must(await admin.from('heroes').select('status').eq('id',heroes.find(h=>h.warband_id===ids[0]).id).single()).status).toBe('active');
 expect(errors).toEqual([]);
 console.log('PASS: mobile agreed exchange returns a captured Wardog and enemy Hero together, preserving companion notes and resolving both cases.');
