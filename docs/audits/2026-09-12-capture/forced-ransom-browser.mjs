...
try {
 ...
 must(await admin.from('henchman_groups').update({xp:1}).eq('id',group.id));
 ...
 await p.getByRole('combobox',{name:'Agree what happens to the captive'}).selectOption('ransom');
 await p.getByLabel('Agreed ransom (gc)',{exact:true}).fill('25');
 ...
 const returned=must(await admin.from('henchman_groups').select('id,size,xp').eq('warband_id',ids[1]).neq('id',group.id).single());
 expect(returned).toMatchObject({size:1,xp:0});
 expect(must(await admin.from('henchman_groups').select('size,xp').eq('id',group.id).single())).toMatchObject({size:1,xp:1});
 expect(must(await admin.from('items').select('quantity').eq('holder_id',returned.id).eq('item_rules_id','axe').single()).quantity).toBe(2);
 expect(must(await admin.from('warbands').select('gold').eq('id',ids[0]).single()).gold).toBe(125);
 expect(must(await admin.from('warbands').select('gold').eq('id',ids[1]).single()).gold).toBe(75);
 expect(errors).toEqual([]);
 console.log('PASS: mobile ransom pays25gc and returns the captive as a separate group without overwriting the advanced survivors.');
