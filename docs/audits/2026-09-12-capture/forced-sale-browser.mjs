...
try {
 ...
 await p.getByRole('combobox',{name:'Agree what happens to the captive'}).selectOption('sell');
 await p.getByRole('button',{name:'Roll D6',exact:true}).click();
 await expect(p.getByLabel('Sale die result',{exact:true})).toBeVisible();
 const original=Number(await p.getByLabel('Sale die result',{exact:true}).inputValue());
 const changed=original===1?6:1;
 await p.getByLabel('Sale die result',{exact:true}).fill(String(changed));
 ...
 const resolved=must(await admin.from('captive_cases').select('resolution_kind,resolution_message').eq('match_id',match).single());
 expect(resolved.resolution_kind).toBe('sell');
 expect(resolved.resolution_message).toContain(`app rolled ${original}, changed by the player`);
 expect(must(await admin.from('henchman_groups').select('size').eq('id',group.id).single()).size).toBe(1);
 expect(must(await admin.from('warbands').select('gold').eq('id',ids[0]).single()).gold).toBe(100+changed*5);
 expect(must(await admin.from('items').select('quantity').eq('warband_id',ids[0]).eq('holder_type','stash').eq('item_rules_id','axe').single()).quantity).toBe(2);
 expect(errors).toEqual([]);
 console.log('PASS: mobile captive sale preserves edited app dice, pays the exact gold and transfers two axes without returning the model.');
