...
try {
 ...
 await expect(count.locator('span')).toHaveText('1');
 await expect(count.getByRole('button',{name:'Fewer QA Captives out of action',exact:true})).toBeDisabled();
 await expect.poll(async()=>must(await admin.from('battle_events').select('reverted_at,payload').eq('match_id',match)).filter(e=>e.payload.metadata_only&&!e.reverted_at).length).toBe(0);
 expect(errors).toEqual([]);
 console.log('PASS: mobile manual capture marker records Subjugator attribution without duplicating the tally, persists through reload, and reverts when its manual casualty is removed.');
