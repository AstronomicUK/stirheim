...
try {
 ...
 await p.getByRole('button',{name:'Next',exact:true}).click();
 await expect(p.getByText('Casualty 2: captured by Subjugator of Mankind; no injury roll.',{exact:true})).toBeVisible();
 const axe=must(await admin.from('items').select('id').eq('holder_id',group.id).eq('item_rules_id','axe').single());
 const captured=must(await admin.from('battle_events').select('id,payload').eq('match_id',match)).sort((a,b)=>Number(Boolean(a.payload.metadata_only))-Number(Boolean(b.payload.metadata_only))).map((e,i)=>({modelIndex:i+1,eventId:e.id,captorWarbandId:ids[0],reason:'subjugator',kit:[{sourceItemId:axe.id,itemId:'axe',quantity:2}]}));
 ...
 console.log('PASS: mixed app and manual Subjugator casualties appear in injuries without dice and create two distinct cases with exact equipment.');
