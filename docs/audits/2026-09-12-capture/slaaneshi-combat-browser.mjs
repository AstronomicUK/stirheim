...
try {
 ...
 expect(errors).toEqual([]);console.log((process.env.TABLETOP_HOLD==='1'?'TABLETOP ':'ROLLER ')+'PASS: actual mobile Man-Catcher attack → selected model 2 held → end confirmation → report without OOA/injury dice → exact captive and kit.');
