...
try {
 ...
 const handler=heroes.find(h=>h.warband_id===ids[1]);
 const dogs=must(await admin.from('items').insert({warband_id:ids[1],holder_type:'hero',holder_id:handler.id,item_rules_id:'wardogs',quantity:2,notes:'Trained pair'}).select('id').single());
 const animalId=`animal:${handler.id}:wardogs:2`;
 ...
 await p.getByLabel('Held enemy',{exact:true}).selectOption(animalId);
 ...
 await p.getByRole('combobox',{name:'Enemy model',exact:true}).selectOption(animalId);
 ...
 const held=must(await admin.from('slaaneshi_holds').select('*').eq('match_id',match).single());expect(held.target_id).toBe(animalId);
 const event=must(await admin.from('battle_events').select('id,payload,summary').eq('id',held.source_event_id).single());expect(event.payload).toMatchObject({out_of_action:false,kill:false,wounds_lost:1});expect(event.summary).toContain('Wardog');
 ...
 await expect(p.getByText(/Held by the Slaaneshi Man-Catcher at battle end; no injury roll/)).toBeVisible();
 ...
 must(await player.rpc('submit_battle_report',{p_match_id:match,p_warband_id:ids[1],p_report:{result:'lost',ooa:[],applied:{item_patches:[{id:dogs.id,quantity:1}],captured_companions:[{sourceItemId:dogs.id,holderId:handler.id,itemId:'wardogs',animalId,eventId:event.id,captorWarbandId:ids[0],reason:'slaaneshi_lock'}]}}}));
 expect(must(await admin.from('captive_cases').select('id').eq('match_id',match))).toHaveLength(1);
 expect(errors).toEqual([]);console.log((process.env.TABLETOP_HOLD==='1'?'TABLETOP ':'ROLLER ')+'PASS: actual mobile Man-Catcher attack → second wardog held → end confirmation → report without OOA/injury dice → exact companion capture.');
