...
try {
 ...
 const wielder=heroes.find(h=>h.warband_id===ids[0]);
 const attack=()=>player.from('battle_events').insert({match_id:match,actor_id:uid,actor_warband_id:ids[0],kind:'attack',summary:'Whipmaster holds model 2',payload:{attacker_warband_id:ids[0],attacker_id:wielder.id,attacker_kind:'hero',attacker_name:'QA Captain',target_warband_id:ids[1],target_id:group.id,target_kind:'group',target_name:'QA Captives',target_size:4,wounds_lost:1,out_of_action:false,kill:false,outcome:'Knocked down',slaaneshi_lock:{weaponId:'slaaneshi_man_catcher',modelIndex:1}}});
 must(await attack());
 await p.goto(`http://127.0.0.1:5193/matches/${match}/battle`);
 const panel=p.getByRole('region',{name:'Man-Catcher holds'});
 await expect(panel.getByText('QA Captives · model 2',{exact:true})).toBeVisible();
 await panel.getByRole('button',{name:'Record release',exact:true}).click();
 const dialog=p.getByRole('dialog');await dialog.getByLabel('Why did the hold end?',{exact:true}).selectOption('magicEscape');await dialog.getByLabel('Optional note',{exact:true}).fill('Escape spell agreed at the table');await dialog.getByRole('button',{name:'Record release',exact:true}).click();
 await expect(panel).toHaveCount(0);
 const released=must(await admin.from('slaaneshi_holds').select('release_reason,history').eq('match_id',match));expect(released[0].release_reason).toBe('magicEscape');
 must(await attack());await p.reload();await expect(panel).toBeVisible();
 await p.getByRole('button',{name:'Battle over',exact:false}).first().click();
 const end=p.getByRole('dialog');await end.getByRole('button',{name:'Still held at battle end',exact:true}).click();
 await expect(end.getByRole('button',{name:'Confirmed',exact:true})).toBeDisabled();
 expect(await p.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);
 await p.screenshot({path:'/tmp/stirheim-slaaneshi-hold-mobile.png',fullPage:true});
 expect(errors).toEqual([]);console.log('PASS: mobile shared hold shows exactly model 2, records magic release, and confirms a subsequent hold at battle end.');
