...
try {
 ...
 await p.reload();await p.getByRole('button',{name:'Melee Attack',exact:false}).first().click();
 await p.getByRole('combobox',{name:'Enemy model',exact:true}).selectOption(group.id);
 await p.getByRole('button',{name:'Roll it through',exact:true}).click();
 await p.getByRole('dialog').getByRole('combobox',{name:'Weapon',exact:true}).selectOption({label:'Dagger (1)'});
 await expect(p.getByRole('button',{name:'Begin attacks',exact:true})).toBeDisabled();
 await p.getByRole('button',{name:'Release hold and use this weapon',exact:true}).click();
 await expect(p.getByRole('button',{name:'Begin attacks',exact:true})).toBeEnabled();
 expect(must(await admin.from('slaaneshi_holds').select('*').eq('id',held.id).single()).release_reason).toBe('weaponSwitched');
 expect(errors).toEqual([]);console.log('PASS: mobile weapon switch requires release, persists it and then permits attack.');
