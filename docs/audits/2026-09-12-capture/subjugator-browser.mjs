...
try {
 ...
 await p.getByRole('button',{name:'Next',exact:true}).click();
 await expect(p.getByText('QA Defender: Captured',{exact:true})).toBeVisible();
 await expect(p.getByText(/Subjugator of Mankind: captured by QA Captain; no Serious Injury roll/)).toBeVisible();
 expect(errors).toEqual([]);
 console.log('PASS: mobile combat logs the Subjugator capture, then the victim report shows Captured without asking for an injury roll.');
