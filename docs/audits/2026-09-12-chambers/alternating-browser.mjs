...
try {
 ...
 await expect(p.locator('.chamber-count')).toHaveCount(3);
 await expect(p.locator('.chamber-count').first()).toContainText('2 / 2 loaded');
 await p.getByRole('radio',{name:'Both barrels',exact:true}).last().click();
 ...
 await expect(p.locator('.chamber-count').nth(1)).toContainText('2 / 2 loaded');
 await p.getByRole('button',{name:'Close',exact:true}).last().click();
 await p.getByRole('button',{name:/View Rosters/}).click();
 await expect(p.locator('.roster-chamber')).toHaveCount(2);
