...
try {
 ...
 must(await admin.from('items').insert(heroes.flatMap(h=>['double_barrelled_pistol','dagger'].map(item_rules_id=>({warband_id:h.warband_id,holder_type:'hero',holder_id:h.id,item_rules_id,quantity:2})))));
 ...
 await p.getByRole('radio',{name:'Both barrels',exact:true}).first().click();
 await p.getByRole('button',{name:'Record shot at the table',exact:true}).first().click();
 await expect(p.locator('.chamber-count').first()).toContainText('0 / 2 loaded');
 ...
 for(const [turn,barrels] of [[2,1],[3,2],[4,1]]) {
  ...
  await p.getByRole('radio',{name:barrels===2?'Both barrels':'1 barrel',exact:true}).first().click();
  await p.getByRole('button',{name:'Record shot at the table',exact:true}).first().click();
 ...
 console.log('PASS: actual mobile tabletop 2/1/2/1 sequence and mini roster persist without a non-firing reload action.');
