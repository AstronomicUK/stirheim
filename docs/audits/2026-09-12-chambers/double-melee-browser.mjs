...
try {
 ...
 await p.getByRole('dialog').getByRole('combobox',{name:'Other hand',exact:true}).selectOption({label:'Double-barrelled Pistol (close combat)'});
 ...
 await p.getByText('Enter tabletop dice instead',{exact:true}).click();
 await p.getByRole('button',{name:/to wound: 1$/}).click();
 await expect(p.getByText('Second barrel: to wound',{exact:true}).first()).toBeVisible();
 await p.getByText('Enter tabletop dice instead',{exact:true}).click();
 await p.getByRole('button',{name:/to wound: 4$/}).click();
 await p.getByText('Enter tabletop dice instead',{exact:true}).click();
 await p.getByRole('button',{name:/injury.*: 6$/i}).click();
 await expect.poll(async()=>must(await admin.from('battle_sessions').select('live_state').eq('match_id',match).eq('warband_id',ids[0]).maybeSingle())?.live_state?.pistolCombatUses?.length).toBe(1);
 const live=must(await admin.from('battle_sessions').select('live_state').eq('match_id',match).eq('warband_id',ids[0]).single()).live_state;
 expect(live.blackpowderShots).toHaveLength(1);
 expect(live.blackpowderShots[0].barrels).toBe(2);
 ...
 console.log('PASS: double-barrel melee resolves two wounds, OOA stops the brace before the second pistol, only the fired pistol loses its chambers after refresh.');
