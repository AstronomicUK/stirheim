...
try {
 ...
 must(await admin.from('items').insert(heroes.flatMap(h=>['ostlander_double_barrelled_pistol','dagger'].map(item_rules_id=>({warband_id:h.warband_id,holder_type:'hero',holder_id:h.id,item_rules_id,quantity:2})))));
 ...
 await expect(p.getByText('2 loaded barrels will fire',{exact:true})).toBeVisible();
 ...
 await p.getByRole('button',{name:/to hit: 4$/i}).click();
 await expect(p.getByText('First barrel: to wound',{exact:true}).first()).toBeVisible();
 await p.getByText('Enter tabletop dice instead',{exact:true}).click();
 await p.getByRole('button',{name:/to wound: 1$/i}).click();
 ...
 await p.getByText('Enter tabletop dice instead',{exact:true}).click();
 await p.getByRole('button',{name:/injury.*: 1$/i}).click();
 ...
 await p.screenshot({path:'/tmp/stirheim-ostland-chambers-mobile.png',fullPage:true});
 ...
 console.log('PASS: one hit roll, first barrel fails to wound, second barrel wounds and rolls injury; spent chambers, reload and mini roster persist without overflow or page errors.');
