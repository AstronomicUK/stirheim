...
try {
 ...
 await expect(p.locator('.roster-chamber .chamber-ball')).toHaveCount(1);
 await expect(p.locator('.roster-chamber-state').first()).toContainText('next fire');
 await p.screenshot({path:'/tmp/stirheim-roster-chambers-mobile.png',fullPage:true});
