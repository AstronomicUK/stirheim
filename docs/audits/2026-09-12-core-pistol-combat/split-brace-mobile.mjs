...
try {
 ...
 await p.getByRole('button',{name:'Fewer attacks at QA Defender',exact:true}).click();
 ...
 await p.getByRole('button',{name:'Pistol (close combat): to hit: 1',exact:true}).click();
 await p.getByRole('button',{name:'Close',exact:true}).last().click();
 await p.getByRole('button',{name:'Roll it through',exact:true}).click();
 await p.getByRole('dialog').getByRole('combobox',{name:'Other hand',exact:true}).selectOption('-1');
 await p.getByRole('dialog').getByRole('combobox',{name:'Combat pistol 1',exact:true}).selectOption({index:1});
 await expect(p.getByRole('dialog')).toContainText('Resolve the remaining brace pistol on its own');
 await expect(p.getByRole('button',{name:'Begin attacks',exact:true})).toBeEnabled();
 await p.getByRole('button',{name:'Begin attacks',exact:true}).click();
 await p.getByText('Enter tabletop dice instead',{exact:true}).click();
 await p.getByRole('button',{name:'Pistol (close combat): to hit: 1',exact:true}).click();
