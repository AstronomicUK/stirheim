...
try {
 ...
 const sent=must(await admin.from('app_notifications').select('id,read_at').eq('user_id',users[0].id));expect(sent).toHaveLength(1);expect(sent[0].read_at).not.toBeNull();
