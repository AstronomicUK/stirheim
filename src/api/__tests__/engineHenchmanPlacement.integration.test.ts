...
describe.skipIf(!enabled)('Engine of Chaos custody for Man-catcher henchmen (#229 / #95, migration 104)',()=>{
 ...
 it('returns a rescued henchman as one original-profile model without returning confiscated kit',async()=>{
  check(await dwarf.rpc('submit_battle_report',{p_match_id:match,p_warband_id:cw,p_report:{result:'won',applied:{}}}))
  const [c]=await cases(),owner=await detail(vw),captor=await detail(cw)
  const built=buildEnginePlacementProposal({item:c as CaptiveCase,owner,captor,engineId:engine})
  const placement=check(await propose(dwarf,c.id,built.choice,diffRoster(owner,built.nextOwner),diffRoster(captor,built.nextCaptor)))
  check(await reik.rpc('respond_captive_proposal',{p_proposal_id:placement,p_action:'accept'}))
  const [prisoner]=await prisoners()
  const rescueMatch=check(await admin.from('matches').insert({campaign_id:campaign,created_by:users[2],state:'completed'}).select('id').single()).id
  check(await admin.from('match_participants').insert([vw,cw].map(warband_id=>({match_id:rescueMatch,warband_id,accepted_at:new Date().toISOString()}))))
  const rescue=check(await admin.from('engine_rescue_battles').insert({match_id:rescueMatch,engine_id:engine,state:{engineId:engine,holderWarbandId:cw,destroyed:true,holderRouted:false,keys:[],prisoners:[{id:prisoner.id,name:prisoner.name,state:'escaped',formerWarbandId:vw,large:false}]}}).select('id').single())
  const engineNow=check(await admin.from('engine_of_chaos_units').select('updated_at').eq('id',engine).single())
  expect((await dwarf.rpc('dispatch_engine',{p_engine_id:engine,p_escort_hero_id:gaoler,p_prisoner_ids:[prisoner.id],p_expected_updated_at:engineNow.updated_at})).error?.message).toMatch(/freed prisoners/)
  const proposal=check(await reik.rpc('propose_engine_rescue_return',{p_rescue_id:rescue.id,p_prisoner_id:prisoner.id}))
  expect(check(await admin.from('henchman_groups').select('id').eq('warband_id',vw))).toHaveLength(1)
  check(await dwarf.rpc('respond_captive_proposal',{p_proposal_id:proposal,p_action:'accept'}))
  const returned=check(await admin.from('henchman_groups').select('*').eq('warband_id',vw).neq('id',group))
  expect(returned).toHaveLength(1);expect(returned[0]).toMatchObject({size:1,xp:2,stats})
  expect(check(await admin.from('items').select('id').eq('holder_id',returned[0].id))).toEqual([])
  expect((await stash()).map((i:any)=>i.item_rules_id).sort()).toEqual(['shield','sword'])
  check(await gm.rpc('reverse_captive_resolution',{p_case_id:c.id,p_reason:'Corrected the recorded rescue',p_release_only:false}))
  expect(check(await admin.from('henchman_groups').select('id').eq('warband_id',vw))).toHaveLength(1)
