...
export function beginAnimosity(sheet: BattleLiveState, input: Pick<AnimosityTest, 'id' | 'warriorId' | 'modelIndex' | 'name' | 'turnKey' | 'at' | 'conflictingRule'>): BattleLiveState {
...
export function acceptAnimosityLeadership(sheet:BattleLiveState,id:string,dice:[number,number]):BattleLiveState {
 ...
 return save(recordAnimosityLeadership(sheet,test),{...test,triggerDice:dice,stage:failed?'effect':'done',outcome:failed?undefined:'clear'},`${provenance}. Leadership ${test.leadership}: ${failed?'failed; roll the Animosity result die':'passed; acts normally'}.`)
}
