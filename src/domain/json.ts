...
export const henchmanCampaignStateSchema = z.object({inheritedSkillIds:z.array(z.string().min(1).max(120)).max(100).optional(),lycanthropes:z.array(z.object({id:z.string(),name:z.string(),contractedAfter:z.string()})).max(5).optional(),raidAbsences:z.array(z.object({count:z.number().int().min(1).max(5),games:z.number().int().min(1).max(2)})).optional(),fanaticBattleMatch:z.string().optional(),fanaticSittingOut:z.boolean().optional(),fanaticSplitFrom:z.string().optional(),permanentStupidity:z.boolean().optional(),upkeepOwedAfter:z.string().optional(),upkeepPaidAfter:z.string().optional(),cheapTrollFeed:z.boolean().optional()});
