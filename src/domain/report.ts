...
export const henchmanInjuryLineSchema = z.object({
  ...
  equipmentLost: z.array(z.object({sourceItemId:uuidSchema,quantity:z.number().int().min(0),manual:z.boolean().optional()})).optional(),
...
export const explorationRecordSchema = z.object({
  enginePrisoners: z.object({count:z.number().int().min(1).max(3),originalRoll:z.number().int().min(1).max(3).nullable(),maximumFinds:z.boolean()}).optional(),
...
export const reportAppliedSchema = z.object({
  ...
  abomination_rewards: z.array(z.object({group_id:z.string().uuid(),model_index:z.number().int().min(0),recipient_id:z.string().uuid(),model_name:z.string().trim().min(1).max(120)})).max(20).optional(),
  ...
  captured_companions: z.array(z.object({sourceItemId:z.string().uuid(),holderId:z.string().uuid(),itemId:z.enum(['wardogs','gnoblar_fighter']),animalId:z.string(),eventId:z.string().uuid(),captorWarbandId:z.string().uuid(),reason:z.enum(['subjugator','slaaneshi_lock'])})).optional(),
