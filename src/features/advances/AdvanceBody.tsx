...
function RollStep({ draft, plan, update }: StepProps<HeroPlan | GroupPlan>) {
  if ('protectoratePrayerChoice' in plan && plan.protectoratePrayerChoice) return <>
    <Notice title="The new leader’s first advance">Choose a {plan.successionMagicLabel} from the list, or roll a normal advancement. This choice is available only for this advance.</Notice>
    <SelectField label="First advance as leader" value={draft.protectorateChoice ?? ''} onChange={e => update(d => ({...d,protectorateChoice:e.target.value as 'prayer' | 'roll',spellId:null}))}>
      <option value="">Choose how to advance</option><option value="prayer">Take a {plan.successionMagicLabel} instead of rolling</option><option value="roll">Roll a normal advancement</option>
    </SelectField>
    {draft.protectorateChoice === 'prayer' ? <SelectField label={`${plan.successionMagicLabel === 'prayer' ? 'Prayer' : 'Spell'} to learn`} value={draft.spellId ?? ''} onChange={e => update(d => setSpell(d,e.target.value))}><option value="">Choose a {plan.successionMagicLabel}</option>{plan.spells.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</SelectField> : draft.protectorateChoice === 'roll' ? <NormalRollStep draft={draft} plan={plan} update={update}/> : null}
  </>
