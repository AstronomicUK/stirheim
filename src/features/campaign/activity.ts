  ...
  for (const key of keys) {
    ...
    if ((key === 'flags' || key === 'settings' || key === 'stat_increases') && (asRow(a) || asRow(b))) {
      ...
      for (const child of new Set([...Object.keys(oldFields), ...Object.keys(newFields)])) {
        ...
        if (child === 'dreamerCertification') { const result=newFields[child] as {die?:number;history?:string[]}|undefined; out.push({label:'Dreamer certification',before:'Previous attempt',after:result?`D6 ${result.die}: ${Number(result.die)>=4?'genuine Dreamer certified':'rejected; retry after next battle'}. ${(result.history??[]).join('; ')}`:'None'}); continue }
