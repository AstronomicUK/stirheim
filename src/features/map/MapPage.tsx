...
function MapView({ detail }: { detail: CampaignDetail }) {
  ...
  const [viewAs, setViewAs] = useState<string>(mine[0]?.warband_id ?? '')
  ...
  return (
    <>
      ...
      >
          ...
          {members.length > 0 ? (
            <div className="flex flex-wrap items-end gap-3">
              <div className="min-w-56 flex-1">
                <SelectField label="Show where a warband can fight" value={viewAs} onChange={(e) => setViewAs(e.target.value)}>
                  <option value="">Everyone (no highlight)</option>
                  {warbands
              ...
              {chosen && reach ? (
                <p className="pb-2 text-sm text-ink-dim">
                  {reach.reachable.size} district{reach.reachable.size === 1 ? '' : 's'} in reach, {reach.explored.size} explored from a gate.
                </p>
              ) : null}
            </div>
          ) : null}
        ...
        {chosen ? <WarbandOutlook warband={chosen} state={state} /> : null}
              ...
              return (
                <button key={d.id} type="button" onClick={() => setSelectedId(d.id)} className="flex items-start justify-between gap-3 py-2 text-left hover:bg-surface-high/50">
                  ...
                  <span className="flex shrink-0 flex-col items-end text-xs text-ink-dim">
                    ...
                    {viewAs ? <span>{toll === null ? 'Out of reach' : toll > 0 ? `In reach, ${toll} gc gate toll` : 'In reach'}</span> : null}
                  </span>
                </button>
              )
            })}
