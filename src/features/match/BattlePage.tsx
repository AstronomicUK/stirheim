    ...
    <>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-6">
          ...
          {sideTab === 'mine' ? <MyWarbandTab reloadRule={houseRules.doubleBarrelSkillReload} tabletopAmmunition={!inApp} roster={roster} items={items} healingHerbsSingleUse={houseRules.healingHerbsSingleUse} template={template} sheet={shown} rawSheet={handle.sheet} edit={handle.edit} readOnly={readOnly} events={events} matchId={match.id} others={others} /> : null}
          {sideTab === 'enemy' ? <EnemyView matchId={match.id} participants={others} sessions={sessions} events={events} turn={shown.turn} /> : null}
          {sideTab === 'cast' ? <CastTab onLogEvent={onLogEvent} events={events} sessions={sessions} matchId={match.id} roster={roster} template={template} others={others} sheet={shown} readOnly={readOnly} edit={readOnly ? undefined : handle.edit} /> : null}
          {sideTab === 'fight' && inApp ? (
            <FightTab
              items={items}
              matchId={match.id}
