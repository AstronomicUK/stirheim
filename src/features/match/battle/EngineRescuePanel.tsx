...
import {useEngineRescues,useStartEngineRescue,useEngineRescueAction,useCorrectEngineRescue} from '../../../api/engineRescue'
 ...
 return <section aria-label="Engine rescue" className="space-y-3">
  ...
  {selected?<Sheet open title={engine?.name??'Engine rescue'} onClose={()=>setSelected(null)} footer={record&&editable?<Button block disabled={!actionReady||!mayRecord||save.isPending||note.trim().length<3} onClick={submit}>Record confirmed event</Button>:undefined}>
   ...
   {record&&editable?<div className="space-y-3">
    ...
    {!mayRecord?<p className="text-xs text-ink-dim">{type==='gaolerOut'&&!model?'With the key holder unknown, the Engine owner or GM must record the Gaoler’s loss. If your model took the keys, select that model above.':'The relevant model’s player, the Engine owner or the GM must confirm this action.'}</p>:null}
   </div>:null}
   ...
   {record?.history.length?<div className="mt-5 border-t border-border pt-3"><h3 className="text-sm font-semibold">Rescue history</h3><ul className="mt-2 space-y-2 text-sm text-ink-dim">{record.history.filter(h=>h.note).map((h,i)=><li key={i} className={h.revertedAt?'line-through opacity-60':undefined}>{h.note}</li>)}</ul></div>:null}
   {record&&record.history.some(h=>h.action&&!h.revertedAt)&&(isGm||record.history.filter(h=>h.action&&!h.revertedAt).at(-1)?.by===userId)?<details className="mt-4 border-t border-border pt-3"><summary className="cursor-pointer text-sm text-ink-dim">Correct a recorded event</summary><p className="my-2 text-xs text-ink-dim">Undo the latest remaining event first. Supporting returns must be withdrawn or reversed before correcting their rescue.</p><TextField label="Reason for correction" value={correction} onChange={e=>setCorrection(e.target.value)}/><Button variant="secondary" disabled={correct.isPending||correction.trim().length<5} onClick={()=>correct.mutate({record,reason:correction},{onSuccess:()=>setCorrection('')})}>Undo latest rescue event</Button>{correct.error?<Notice tone="error">{correct.error.message}</Notice>:null}</details>:null}
  </Sheet>:null}
 </section>
}
