...
export function SpellDamage({ spellName, onLog }: { spellName: string; onLog: (lines: LogLine[]) => void }) {
  ...
  const [injuryRule, setInjuryRule] = useState('normal')
  ...
  return <details className="border-t border-border pt-3"><summary className="cursor-pointer text-sm text-brass">Resolve spell damage</summary>
    <div className="mt-3 flex flex-col gap-3">
      ...
      {!run ? <>
        ...
        <SelectField label="Target’s injury rule" value={injuryRule} onChange={e=>setInjuryRule(e.target.value)}><option value="normal">Normal injury chart</option><option value="veskit">Veskit: ignore knocked down and stunned</option></SelectField>
        <Button variant="secondary" disabled={!target.trim() || !hits || hits < 1 || hits > 30 || !Number.isInteger(hits) || !wounds || wounds < 1 || !Number.isInteger(wounds)} onClick={() => {
          ...
          onLog([{ text: `Spell damage: ${hits} hit(s) on ${target}; ${wounds} Wounds remaining, wound ${toWound}+, armour ${armour}, ward ${ward}; injury rule ${injuryRule}; no critical hits.`, tone: 'neutral' }, ...next.log]); setRun(next)
