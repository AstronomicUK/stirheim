...
import { reloadTurnsFor } from '../fight/reloadRules'
...
export function TabletopChambers({warrior,items,events,sheet,ownTurn,readOnly,edit}:{warrior:Combatant;items:readonly ItemRow[];events:readonly BattleEventRow[];sheet:BattleLiveState;ownTurn:number;readOnly:boolean;edit:(fn:(s:BattleLiveState)=>BattleLiveState)=>void}) {
 ...
 return <div className="px-4 pb-3">
  ...
  <Sheet open={open} onClose={()=>setOpen(false)} title={`${warrior.name}: ammunition`}>
   ...
   {copies.some(copy=>copy.snapshot.weaponId.includes('double_barrelled'))?<div className="flex flex-col gap-2 py-3">
    ...
    <Button variant="secondary" disabled={readOnly||uneven||!reloadConfirmed} onClick={()=>{const id=crypto.randomUUID(),at=new Date().toISOString();change(s=>reloadDoubleBarrels(s,copies.filter(copy=>copy.snapshot.weaponId.includes('double_barrelled')).map(gun),ownTurn,id,at));setReloadConfirmed(false)}}>Reload spent barrels</Button>
   </div>:null}
