...
export function FightTab({ items = [], matchId, roster, template, others, sessions, houseRules, sheet, events, readOnly, onLogEvent, edit, boosts, startWith = 'melee' }: FightTabProps) {
  ...
  const [chosenBarrels, setBarrels] = useState<1|2>(1)
  ...
  const isDoubleBarrel = !selfDamage && primary?.type === 'ranged' && primary.special.some(rule=>['doubleBarrelledOptionalSecondWoundRollPerHit','doubleBarrelledTwoHitsPerSuccessfulShot'].includes(rule))
  ...
  const doubleAll = isDoubleBarrel && attacker ? ['double_barrelled_pistol','double_barrelled_duelling_pistol','double_barrelled_handgun','ostlander_double_barrelled_pistol','ostlander_double_barrelled_hunting_rifle'].flatMap(id=>physicalWeaponChoices(items,events,roster.id,attacker.id,id)) : []
  ...
  const [combatBarrelChoices,setCombatBarrelChoices] = useState<Record<string,1|2>>({})
  ...
  const combatPistols = combatPistolWeapons.map((weapon, index) => {
    ...
    const chamber = attacker && selected && weapon.physicalWeaponId?.includes('double_barrelled') ? doubleBarrelState(sheet,{warriorId:attacker.id,modelIndex:pistolSlot,weaponKey:physicalGunKey(selected.snapshot,''),name:weapon.name},Number(ownTurnKey.split(':').at(-1))) : null
    const count:1|2|undefined = chamber ? weapon.physicalWeaponId?.startsWith('ostlander') ? chamber.loaded===1?1:2 : combatBarrelChoices[selected!.key]??1 : undefined
    const block = !attacker || !selected ? 'Select an intact pistol carried by this model.' : pistolCombatBlock(sheet, combatModelKey, attacker.id, selected.snapshot, combatPistolMode, phaseKey, Number(ownTurnKey.split(':').at(-1)),pistolSlot) ?? (count && chamber && count>chamber.loaded?'Choose only loaded barrels.':null)
    return { weapon, options, selected, field, block, chamber, barrels:count, handIndex:primary===weapon && index===0?0:1 }
  })
  ...
  const attackerPreBattle: PreBattleEffect[] = attackerKit && attacker ? [...preBattleWithRolls(attackerKit.consumables, usedIds.filter(id => id !== 'black_lotus' && id !== 'dark_venom'), itemRollsBy(sheet, attacker.id)), ...recordedPoisonEffects(sheet, attacker.id)] : []
  const defenderUsed = defender && defenderSession ? itemsUsedBy(defenderSession.live_state, defender.id) : []
  const defenderPreBattle: PreBattleEffect[] = defenderKit ? preBattleWithRolls(defenderKit.consumables, defenderUsed.filter(id => id !== 'black_lotus' && id !== 'dark_venom'), defender && defenderSession ? itemRollsBy(defenderSession.live_state, defender.id) : {}) : []

  const hasPoison = Boolean(attacker && sheet.poisonApplications.some(use => use.warriorId === attacker.id && !use.correction))
  const tailKey = attackerKit?.tailWeapon?.choiceId
  const tailConflict = Boolean(tailKey && primary?.special.includes('cumbersomeNoOtherWeapons'))
  const poisonSeen = new Set<string>(tailKey ? [tailKey] : [])
  const poisonBindings = (primary ? [primary, ...(offHand && offHandValid ? [offHand] : [])] : []).map((weapon,index) => {
    const choices = attacker ? physicalWeaponChoices(items,events,roster.id,attacker.id,weapon.id) : []
    const chosen = choices.find(choice => choice.key === physicalSelection[`${attacker?.id}:${index}`]) ?? choices.find(choice => !poisonSeen.has(choice.key))
    if (chosen) poisonSeen.add(chosen.key)
    return chosen?.key
  })
  ...
  const ostlandBarrels = primary?.special.includes('doubleBarrelledTwoHitsPerSuccessfulShot') ?? false
  const barrels:1|2 = ostlandBarrels ? doubleState?.loaded===1 ? 1 : 2 : chosenBarrels
  ...
  const duplicateWeapon = (swordBreaker || hasPoison || Boolean(tailKey)) && !primary?.paired && physicalBindings.some(binding => binding.duplicate)
  ...
  const setupControls = (locked: boolean) => attacker && current && primary ? (
    <>
      ...
      {isDoubleBarrel && doubleState ? <ChamberDisplay name={primary.name} copy={doubleHeld?.label} capacity={2} loaded={doubleState.loaded} ready={!doubleState.block&&!doubleUneven} status={doubleState.block??(ostlandBarrels?`${barrels} loaded barrel${barrels===1?'':'s'} will fire`:'Choose how many barrels to fire')} barrels={barrels} onBarrels={ostlandBarrels?undefined:setBarrels} disabled={locked||readOnly}>
      ...
      {combatPistols.length && attacker ? <Notice title="Pistols in this combat">
        ...
        {combatPistols.map((p,index)=><div key={p.field} className="flex flex-col gap-2"><SelectField label={`Combat pistol ${index+1}`} value={p.selected?.key ?? ''} disabled={locked} onChange={e=>setPhysicalSelection(current=>({...current,[p.field]:e.target.value}))}>{p.options?.copies.map(copy=><option key={copy.key} value={copy.key}>{copy.label}</option>)}</SelectField>{p.chamber&&p.selected?<ChamberDisplay name={p.weapon.name} copy={p.selected.label} capacity={2} loaded={p.chamber.loaded} ready={!p.chamber.block} status={p.chamber.block??`${p.barrels} barrel${p.barrels===1?'':'s'} selected for this attack`} barrels={p.barrels} disabled={locked||readOnly} onBarrels={p.weapon.physicalWeaponId?.startsWith('ostlander')?undefined:count=>setCombatBarrelChoices(current=>({...current,[p.selected!.key]:count}))}/>:null}</div>)}
  ...
  return (
    <>
      ...
      {swordBreaker || hasPoison || tailKey ? <Notice tone="warn" title={swordBreaker ? "Sword Breaker opponent" : "Selected weapon copies"}><div className="flex flex-col gap-2"><p>{swordBreaker ? "A successful parry allows a 4+ Trap Blade roll. Select the carried copy at risk; breaking it does not erase hits already rolled." : hasPoison ? "Select the physical copy used in each hand. Only the recorded copy receives its poison bonus." : "The tail weapon is reserved. Select different physical copies for the hands."}</p>{physicalBindings.map((binding, index) => binding.choices.length > 1 && !binding.tailEntry && (!primary?.paired || index === 0) ? <SelectField key={binding.key} label={`Weapon copy for ${index === 0 ? 'main hand' : 'off hand'}`} value={binding.chosen?.key ?? ''} onChange={e => setPhysicalSelection(current => ({ ...current, [binding.key]: e.target.value }))}>{binding.choices.map(choice => <option key={choice.key} value={choice.key}>{choice.label}</option>)}</SelectField> : null)}{duplicateWeapon ? <p>Select different physical copies for the hands{tailKey ? ' and tail' : ''}.</p> : null}</div></Notice> : null}
      {tailConflict ? <Notice tone="warn">This weapon forbids using another weapon. Choose a shield or nothing for the tail before attacking.</Notice> : null}
      {pistol && attacker ? <div className="flex flex-col gap-3">
        {primary?.id !== 'crossbow_pistol' ? <div className="grid gap-3 sm:grid-cols-2">{pistol.copies.map(copy => {
          const blocked = blackpowderBlock(sheet, attacker.id, physicalGunKey(copy.snapshot, copy.key), Number(ownTurnKey.split(':').at(-1)))
          return <div key={copy.key} className={copy.key === pistol.selected?.key && pistol.copies.length > 1 ? 'rounded-xl ring-2 ring-brass' : ''}><ChamberDisplay name={primary?.name ?? 'Pistol'} copy={copy.label} capacity={1} loaded={blocked ? 0 : 1} ready={!blocked} status={blocked ?? 'Loaded'}>{pistol.copies.length > 1 ? <Button variant="secondary" disabled={readOnly || copy.key === pistol.selected?.key} onClick={() => setPhysicalSelection(current => ({ ...current, [`${attacker.id}:pistol:${pistolSlot}`]: copy.key }))}>{copy.key === pistol.selected?.key ? 'Selected pistol' : 'Select this pistol'}</Button> : null}</ChamberDisplay></div>
        })}</div> : null}
        <p>{pistol.blocked ?? `${pistol.remaining} pistol shot${pistol.remaining === 1 ? '' : 's'} available this own turn. Resolve one shot, then choose the next loaded pistol if Pistolier permits another.`}</p>
        {(attacker.groupSize ?? 1) > 1 ? <SelectField label="Firing group member" value={pistolSlot} onChange={event => setPistolModel(Number(event.target.value))}>{Array.from({ length: attacker.groupSize! }, (_, index) => <option key={index} value={index}>Model {index + 1}</option>)}</SelectField> : null}
        {primary?.id === 'crossbow_pistol' && pistol.copies.length ? <SelectField label="Pistol copy" value={pistol.selected?.key ?? ''} onChange={event => setPhysicalSelection(current => ({ ...current, [`${attacker.id}:pistol:${pistolSlot}`]: event.target.value }))}>{pistol.copies.map(copy => <option key={copy.key} value={copy.key}>{copy.label}</option>)}</SelectField> : null}
        <details><summary className="cursor-pointer text-sm">Correct a pistol firing attempt</summary>
          <TextField label="Reason for pistol correction" value={pistolCorrection} onChange={event => setPistolCorrection(event.target.value)} />
          <p className="text-xs">This restores firing availability. Correct earlier attack results separately in the combat log.</p>
      ...
      {isDoubleBarrel && attacker ? <div className="flex flex-col gap-3">
        ...
        {doubleCopies.map(copy=>{
          ...
          return <ChamberDisplay key={copy.key} name={copy.snapshot.name} copy={copy.label} capacity={2} loaded={state.loaded} status={state.block??'Loaded barrels ready'} ready={!state.block&&!doubleUneven} barrels={selected?barrels:undefined} onBarrels={selected&&!ostlandBarrels?setBarrels:undefined} disabled={readOnly||!!rollSetup}>
      ...
      {isCoreReloadGun ? <ChamberDisplay name={primary?.name ?? 'Firearm'} copy={reloadHeld?.label} capacity={1} loaded={reloadHeld && !reloadBlocked ? 1 : 0} ready={!reloadBlocked && !!reloadHeld} status={reloadBlocked ?? 'Loaded and ready to fire'}>
        ...
        {reloadBlocked && edit && !readOnly ? <div className="flex flex-col gap-2"><TextField label="Reason to correct this firing restriction" value={reloadCorrection} onChange={e => setReloadCorrection(e.target.value)} /><Button variant="secondary" disabled={!reloadCorrection.trim()} onClick={() => {
          const shot = sheet.blackpowderShots.filter(s => s.warriorId === attacker?.id && s.weaponKey === reloadKey && !s.correction).at(-1)
          if (shot) edit(state => correctBlackpowderShot(state, shot.id, reloadCorrection))
          setReloadCorrection('')
      ...
      <div className="relative grid grid-cols-2 items-stretch gap-3 lg:gap-8">
        <FightBox icon="battle" title="Attacker" tone="brass">
          ...
          {attacker && edit ? <TailFightingControl warrior={attacker} items={items} events={events} sheet={sheet} readOnly={readOnly} edit={edit} /> : null}
          {attacker && attackerKit && edit ? <PoisonControls key={attacker.id} warrior={attacker} kit={attackerKit} items={items} events={events} sheet={sheet} readOnly={readOnly} edit={edit} /> : null}
          {attacker && attackerKit && attackerKit.consumables.some(c => c.itemId !== 'black_lotus' && c.itemId !== 'dark_venom') ? (
            <fieldset className="flex min-w-0 flex-col gap-0.5">
              <legend className="mb-0.5 text-[10px] uppercase tracking-wider text-ink-dim">Taken this battle</legend>
              {attackerKit.consumables.filter(c => c.itemId !== 'black_lotus' && c.itemId !== 'dark_venom').map((c) => {
      ...
      {odds && attacker && defender ? (
        <>
          ...
          {!rollSetup ? <div className="flex flex-col gap-4 py-3">
            ...
            {holdTargetConflict?<p role="status" className="rounded-md border border-brass p-3 text-sm">This Man-Catcher already holds {wieldersHold?.target_name}. Resolve that hold from the battle sheet before attempting to hold another model.</p>:null}
            ...
            onProgress={edit ? (previous, next, attemptId, rolled) => {
              ...
              if (isCoreReloadGun && !areaTarget && primary) {
                const startsReloadShot = firingAttemptStarted(previous,next)
                if (startsReloadShot) edit(s => recordBlackpowderShot(s, { id: attemptId, warriorId: attacker.id, weaponKey: reloadKey, weaponName: primary.name, heldWeapon: reloadHeld?.snapshot, modelIndex: attacker.kind === 'henchman' && reloadHeld ? Math.floor(reloadHeld.snapshot.copyIndex / (reloadHeld.snapshot.expected.quantity / Math.max(1,attacker.groupSize??1))) : 0, ownTurn: Number(ownTurnKey.split(':').at(-1)), reloadTurns: reloadTurnsFor(primary, attacker.skillIds) ?? 1, experimental: false, at: new Date().toISOString() }, attacker.name))
              }
