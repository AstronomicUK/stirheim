...
import { correctLineShot, declareLineShot, lineShotBlock, unresolvedLineTargets, type BattleLiveState, type BattleEventRow, type LineShot, type LineShotTarget } from '../../../domain'
import { Button, SelectField, Sheet, TextField } from '../../../ui'
import type { Combatant } from './combatants'

export function LineShotControls({ attacker, weaponId, models, sheet, events, ownTurn, mayFire, readOnly, edit, onResolve }: {
  attacker: Combatant; weaponId: LineShot['weaponId']; models: Combatant[]; sheet: BattleLiveState; events: BattleEventRow[];
  ownTurn: number; mayFire: boolean; readOnly: boolean; edit?: (fn: (s: BattleLiveState) => BattleLiveState) => void;
  ...
  return <div className="flex flex-col gap-3 rounded border border-brass p-3 text-xs">
    ...
    {blocked ? <p>{blocked}</p> : <Button variant="secondary" disabled={readOnly || !edit || !mayFire} onClick={() => { setChosen([]); setOpen(true) }}>Choose the models in the line</Button>}
    {shots.map(shot => {
