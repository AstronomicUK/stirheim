...
import { startPigeonLaunch, confirmPigeonLaunch, declarePigeonBlast, pigeonBlastCentre, unresolvedPigeonVictims, type PigeonLaunch, type LineShotTarget, type BattleLiveState, type BattleEventRow } from '../../../domain'
...
export function PigeonLaunchControls({ attacker, models, sheet, events, ownTurn, mayLaunch, readOnly, requiresPermission, edit, onResolve }: {
  ...
  const pending = launches.find(l => l.ownTurn === ownTurn && l.die === undefined)
  ...
  return <div className="flex flex-col gap-3 rounded border border-brass p-3 text-xs">
    ...
    {launches.map(l => <div key={l.id} className="flex flex-col gap-2 border-t border-border pt-2">
      <p>Own turn {l.ownTurn}: {l.die === undefined ? 'launch awaiting confirmation' : l.die === 1 ? 'backfire at the firer' : l.die >= 5 ? `landed on ${l.intendedTarget.name}` : 'exploded harmlessly'}.</p>
    ...
    {test && edit && mayLaunch && !readOnly ? <LaunchTest key={test.id} test={test} requiresPermission={requiresPermission} previous={launches.some(l => l.ownTurn === ownTurn && l.id !== test.id)} edit={edit} close={() => setTest(null)} /> : null}
...
function LaunchTest({ test, previous, requiresPermission, edit, close }: { test: PigeonLaunch; previous: boolean; requiresPermission: boolean; edit: (fn: (s: BattleLiveState) => BattleLiveState) => void; close: () => void }) {
  ...
  const [permission, setPermission] = useState(Boolean(test.permissionNote))
  const allowed = (!previous || Boolean(reason.trim())) && (!requiresPermission || permission)
  const save = { ...test, original, reason, permissionNote: requiresPermission && permission ? 'Blessing of the Lady: player confirmed the required 4+ firing test passed at the table before launch.' : undefined }
  return <Sheet open title="Launch Pigeon Bomb" onClose={close} footer={<Button block disabled={!allowed || die === null} onClick={() => { edit(s => confirmPigeonLaunch(startPigeonLaunch(s, save), test.id, die!)); close() }}>Confirm Pigeon launch</Button>}>
    <div className="flex flex-col gap-4 p-4">
      ...
      {requiresPermission ? <label className="flex items-start gap-2"><input type="checkbox" checked={permission} disabled={original !== undefined} onChange={e => setPermission(e.target.checked)} />Blessing of the Lady: I resolved the required 4+ firing test at the table and passed. On failure, close without launching.</label> : null}
      {previous ? <TextField label="Reason for another Pigeon launch" value={reason} disabled={original !== undefined} onChange={e => setReason(e.target.value)} /> : null}
