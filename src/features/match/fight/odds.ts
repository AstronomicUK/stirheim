...
function oddsNotes(setup: FightSetup, weapons: WeaponOdds[]): string[] {
  ...
  for (const w of weapons) {
    ...
    if (w.input.temperamentalPigeon) notes.push('Pigeon Bomb: a flat launch D6 replaces Ballistic Skill and shooting modifiers. 5–6 lands on target; 2–4 explodes harmlessly; 1 explodes at the firer. These odds cover the selected target only. Resolve other models within 1½ inches of the target, or the firer and nearby models on a backfire, at the table; each takes one Strength 4 hit.')
