...
function oddsNotes(setup: FightSetup, weapons: WeaponOdds[]): string[] {
  ...
  if (primary && primary.attacks === 0) {
    notes.push(setup.context.failedStupidity && setup.attacker.traitIds.includes('stupidity') ? 'Failed Stupidity test: this warrior cannot attack until its next turn.' : setup.context.altFire && setup.primary.special.includes('fireTwiceAtMinus1IfStationaryWithinHalfRange') ? `${setup.primary.name}: two shots require standing still and a target within half range. Turn off two shots to fire once.` : setup.primary.moveOrFire ? `${setup.primary.name} cannot fire in a turn the shooter moved.` : `${setup.primary.name} makes no attacks in this situation.`)
  }
