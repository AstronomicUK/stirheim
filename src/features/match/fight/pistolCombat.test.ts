...
import { beginPistolCombat, useCombatPistol, correctCombatPistol, pistolCombatBlock } from './pistolCombat'
...
it('spends once per combat and shares reload state, surviving a new turn and reload',()=>{
 let s=useCombatPistol(start(),args)
 ...
 expect(useCombatPistol(s,args)).toBe(s)
...
it('allows two distinct brace pistols only in the opening round, never a third or a mixed single/brace reset',()=>{
 let s=useCombatPistol(start(),{...args,mode:'brace'})
 ...
 s=useCombatPistol(s,{...args,id:'shot2',weapon:second,mode:'brace'})
 ...
 expect(pistolCombatBlock(useCombatPistol(start(),args),'hero:0','33333333-3333-4333-8333-333333333333',second,'brace','phase1',1)).toContain('used')
})
it('keeps the crossbow opener separate, without a blackpowder reload charge',()=>{
 ...
 const s=useCombatPistol(start(),{...args,weapon:bow,mode:'crossbow'})
