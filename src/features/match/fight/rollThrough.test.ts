...
it.each([5, 6])('a Powder Keg wound (%s) stops before warrior saves and injuries', die => {
  let state = startPhase([{ ...plan('Torch'), powderKeg: true }], 1, 0)
  state = applyRoll(state, 4)
  expect(state.pending?.kind).toBe('wound')
  state = applyRoll(state, die)
  if (die !== 6) { expect(state.pending?.kind).toBe('powderKegExplosion'); state = applyRoll(state, 4) }
  expect(state.done).toBe(true)
  expect(state.powderKegWounded).toBe(true)
  expect(state.powderKegCritical).toBe(die === 6)
  expect(state.woundsLost).toBe(0)
  expect(state.worst).toBe('wounded')
  expect(state.pending).toBeNull()
})
it('a failed Powder Keg wound and a 6-required wound do not become critical explosions', () => {
  const start = () => applyRoll(startPhase([{ ...plan('Torch', { woundThreshold: 6 }), powderKeg: true }], 1, 0), 4)
  expect(applyRoll(start(), 5).powderKegWounded).toBeUndefined()
  const wounded = applyRoll(applyRoll(start(), 6), 4)
  expect(wounded.powderKegWounded).toBe(true)
  expect(wounded.powderKegCritical).toBe(false)
})

it('retains the second barrel when the first wound fails to explode the keg', () => {
  let state = startPhase([{ ...plan('Double-barrelled Handgun'), barrels: 2, powderKeg: true }], 1, 0)
  state = applyRoll(state, 4)
  state = applyRoll(state, 5)
  expect(state.pending?.kind).toBe('powderKegExplosion')
  state = applyRoll(state, 2)
  expect(state.done).toBe(false)
  expect(state.pending?.kind).toBe('wound')
  state = applyRoll(state, 6)
  expect(state.done).toBe(true)
  expect(state.powderKegCritical).toBe(true)
  expect(state.log.map(l => l.text).join(' ')).toContain('Second barrel')
})
it('stops both-barrel resolution as soon as the keg explodes', () => {
  let state = startPhase([{ ...plan('Double-barrelled Handgun'), barrels: 2, powderKeg: true }], 1, 0)
  state = applyRoll(applyRoll(applyRoll(state, 4), 5), 4)
  expect(state.done).toBe(true)
  expect(state.powderKegExplosionDie).toBe(4)
  expect(state.log.map(l => l.text).join(' ')).not.toContain('Second barrel')
})
it('Chain Shot cannot knock scenery down after a failed ignition wound', () => {
  let state = startPhase([{ ...plan('Chain Shot', { chainShotKnockdown: true }), powderKeg: true }], 1, 0)
  state = applyRoll(applyRoll(state, 4), 1)
  expect(state.done).toBe(true)
  expect(state.pending).toBeNull()
  expect(state.worst).toBe('noWound')
})
