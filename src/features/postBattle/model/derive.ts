...
export interface DerivedReport {
  ...
  handlerAftermath: ReturnType<typeof handlerAftermath>
...
export function deriveInjuries(draft: ReportDraft, participants: Participants, matchId: string, roster?: RosterWarband, perks?: MapPerks | null, scenarioId?: string | null, battleEvents: import('../../../domain').BattleEventRow[] = [], captureLimits?: CaptureLimits, holds: readonly SlaaneshiHold[] = []): InjuriesDerived {
  ...
  const heroes = participants.heroes
    ...
    .map((hero) => {
      ...
      const bearGuard=hero.unitTemplateId==='kislevites_bear_tamer' && participants.groups.some(g=>g.unitTemplateId==='kislevites_trained_bear' && (draft.groupsOut[g.id]??0)===0)
      const squigGuard=hero.unitTemplateId==='night_goblins_web_squig_herder' && participants.groups.some(g=>g.campaignState?.trainedSquig && (draft.groupsOut[g.id]??0)===0 && !draft.trainedSquigGoneWild?.[g.id])
      const rotten=hero.skillIds.some(id=>id.endsWith('_rotten_body')) && draft.censerSelfInjury?.[hero.id]
      const resolution = skip !== undefined ? skippedHero(hero, skip) : rotten ? skippedHero(hero,'Rotten Body: taken out of action by the failed test for this warrior’s own censer; automatically recovers.') : resolveHeroInjuryFlow(hero, draft.heroInjuries[hero.id] ?? { rolls: [], countRoll: null }, matchId, perks, reason, true, bearGuard?'Fiercely Loyal: the surviving Trained Bear protected its Bear Tamer; this result becomes Full Recovery.':squigGuard?'Trainin’: the trained Squig protected its Herder; this result becomes Full Recovery.':undefined)
  ...
  const groups = participants.groups
    ...
    .map((group) => {
      ...
      const dice = draft.groupInjuryDice[group.id]?.count ?? (!flamingTroll && !burning && !hasPlants && henchmanInjuryException(group)?.deadOn.length === 0 ? 0 : injuryCount)
      if (hasPlants) {
      ...
      return { group, outOfAction, dice, resolution: resolveGroupInjuries(group, dice, draft.groupInjuries[group.id] ?? [], draft.constructRepairDice?.[group.id] ?? [], flamingTroll) }
    }).map(row => {
  ...
  const animals = animalFighters(roster ?? { heroes: participants.heroes } as RosterWarband)
    .filter((a) => animalIds.has(a.id)||heldFor(a.id).length>0)
    .map((animal) => {
      const capture = capturesFor(animal.id).find(c=>c.event.payload.capture_reason==='subjugator'||c.event.payload.capture_reason==='slaaneshi_lock')
...
function stepProblems(draft: ReportDraft, injuries: InjuriesDerived, exploration: ExplorationDerived, kit: KitDerived, ctx: ReportContext): Record<StepId, string[]> {
  ...
  const kitPending = kit.prompts.filter(p=>p.itemId!=='treasure_map'&&!p.complete).length
  if (kitPending > 0) problems.injuries.push(`${kitPending} ${kitPending === 1 ? 'roll' : 'rolls'} for kit after the battle still to make.`)
  if (kit.prompts.some(p=>p.itemId==='treasure_map'&&!p.complete)) problems.exploration.push('Finish the Treasure Map destination and reward dice.')
...
export function itemPatchesFor(ctx: ReportContext, draft: ReportDraft): ReportApplied['item_patches'] {
  ...
  for (const [holderId, itemIds] of Object.entries(ctx.itemsUsed ?? {})) {
    for (const itemId of new Set(itemIds)) {
      if (!isConsumable(itemId) || itemId === 'treasure_map') continue
...
function buildApplied(draft: ReportDraft, ctx: ReportContext, participants: Participants, injuries: InjuriesDerived, xp: XpDerived, exploration: ExplorationDerived, kit: KitDerived): ReportApplied {
  ...
  for (const a of injuries.animals) {
    ...
    if (row) {
      ...
      if(a.capture) capturedCompanions.push({sourceItemId:row.id,holderId:a.animal.holderId,itemId:a.animal.itemId as 'wardogs'|'gnoblar_fighter',animalId:a.animal.id,eventId:a.capture.event.id,captorWarbandId:a.capture.event.payload.attacker_warband_id,reason:a.capture.event.payload.capture_reason==='slaaneshi_lock'?'slaaneshi_lock':'subjugator'})
    }
  }
  ...
  for (const removal of effects.removeItems) {
    const row = removal.itemId === 'treasure_map' ? pirateMapChoice(draft,ctx).row : ctx.items.find((r) => r.item_rules_id === removal.itemId && (removal.holderId === null ? r.holder_type === 'stash' : r.holder_id === removal.holderId))
...
export function deriveReport(draft: ReportDraft, ctx: ReportContext): DerivedReport {
  ...
  const exploration = deriveExploration(draft.exploration, explorationRoster, {
    ...
    disabledReason: nonCampaign ? 'Sword of the Herald: no exploration in the agreed non-campaign mode.' : mapChoice.row ? 'Following the Treasure Map instead of exploring the city.' : ctx.scenarioId === 'stake_out' && draft.scenarioRewards?.stakeOut?.mode === 'income-only' ? 'Stake-Out: the table agreed to use the printed fixed income instead of exploration.' : undefined,
  ...
  const handlers = handlerAftermath(ctx,draft,injuries,applied,!nonCampaign)
  ...
  problems.exploration.push(...recruits.problems, ...theft.problems, ...mapChoice.problems)
  if (mapChoice.row && raidRequested > 0) problems.exploration.push('Treasure Map replaces exploration: remove the Raids dice expenditure before continuing.')
