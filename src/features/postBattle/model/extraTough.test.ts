...
import { canUseExtraTough, hasExtraTough, applyExtraToughReroll } from './extraTough'
...
it('replaces the initial injury, preserves original provenance and accepts a worse result', () => {
  ...
  const replaced = applyExtraToughReroll(draft, hero, 11, 'tabletop')
  ...
  expect(applyExtraToughReroll(replaced, hero, 41, 'app')).toBe(replaced)
...
it('requires the learned skill and an initial result without dependent dice or an existing reroll', () => {
  ...
  expect(applyExtraToughReroll(draft, { ...hero, skillIds: [] }, 41, 'app')).toBe(draft)
