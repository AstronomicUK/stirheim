...
export function LocationRecruits({draft,derived,ctx,update}:Pick<StepProps,'draft'|'derived'|'ctx'|'update'>) {
 const plan=locationRecruits(draft,ctx,derived.exploration,derived.injuries)
 if(!plan.kind) return null
 return <Section title="Exploration recruits"><Card className="flex flex-col gap-3 px-4 py-3">
