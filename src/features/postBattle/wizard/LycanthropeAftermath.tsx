...
import {woodsInjuryDraft,transformationItems,type WoodsVictim,type TransformationDraft,type WoodsGroupDraft} from '../model/lycanthropeReport'
...
function GearChoices({items,holderId,groupSize,value,change}:{items:readonly ItemRow[];holderId:string;groupSize?:number;value:TransformationDraft;change:(v:TransformationDraft)=>void}){
 ...
 return <div className="flex flex-col gap-3 rounded border border-border p-3"><p className="text-sm">Review what this model wore or carried when it transformed. Worn equipment is destroyed; dropped weapons may be recovered as their original copies.</p>
 ...
 <label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={!!value.gearReviewed} onChange={e=>change({...value,quantities,gearReviewed:e.target.checked})}/>I have checked the actual carried copies and their fate.</label>
 </div>
}
...
export function LycanthropeAftermath({draft,derived,ctx,update}:Pick<StepProps,'draft'|'derived'|'ctx'|'update'>){
 ...
 {cursed.map(h=><Card key={`return:${h.id}`} className="p-4"><ReturnChoices name={h.name} value={state.returns?.[h.id]??{}} alive={survives(after(h.id))} items={ctx.items} holderId={h.id} change={value=>update(d=>({...d,woods:{...d.woods,returns:{...d.woods?.returns,[h.id]:value}}}))}/></Card>)}
