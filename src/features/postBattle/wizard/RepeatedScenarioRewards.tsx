    ...
    {conditionMet ? <>
      ...
      {entries.map((entry, i) => {
        ...
        return <Card key={i} className="flex flex-col gap-3 px-4 py-3">
          ...
          {rule.tableDice ? <div className="flex flex-wrap gap-3">{Array.from({ length: rule.tableDice }, (_, d) => <DieField key={d} label={`${rule.label} ${i + 1}: table D6 ${d + 1}`} sides={6} rollable value={entry.tableDice?.[d] ?? null} onChange={v => patch(i, e => ({ ...e, dice: [], tableDice: Array.from({ length: rule.tableDice! }, (_, k) => d === k ? v : e.tableDice?.[k] ?? null) }))} />)}</div> : null}
          {row ? <p className="text-sm">{row.label}{quantity ? `: ${quantity.count}D${quantity.sides}${'multiplier' in quantity && quantity.multiplier ? ` × ${quantity.multiplier}` : ''} ${row.quantityIsValue ? 'gc value' : quantityLabel}` : row.shards ? `: ${row.shards} wyrdstone` : row.itemName ? ': one item for the stash' : ': no reward'}.</p> : null}
