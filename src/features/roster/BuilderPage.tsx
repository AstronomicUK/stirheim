import { MarauderTribePicker } from './view/MarauderTribe'
...
function Builder({ draft, template }: { draft: WarbandDraft; template: WarbandTemplate }) {
  ...
  const maxModels = tribeModelLimit(draft, template.composition?.maxModels ?? null)
  ...
  return (
    <BuilderRulesContext.Provider value={houseRules}>
      ...
      {template.id === 'marauders_of_chaos' ? <MarauderTribePicker value={draft.marauderTribe} onChange={tribe => update(d => ({ ...d, marauderTribe: tribe }))} /> : null}
