...
export function EngineRosterSection({ detail, campaignId, userId }: { detail: WarbandDetail; campaignId?: string; userId?: string }) {
  ...
  return <>
    <EngineFleet engines={engines.data ?? []} prisoners={own.map(p => ({ ...p, origin: origin(p.case_id) }))} canEdit={canEdit} journeys={journeys.data} onDispatch={setDispatchId} onJourney={setJourneyId} onReturn={setReturnId} onPrisoner={id => { setReason(''); setSelected(id) }}/>
    {dispatchEngine ? <EngineDispatchSheet engine={dispatchEngine} prisoners={own.filter(p => !p.journey_id)} heroes={detail.roster.heroes.filter(h => !journeys.data?.some(j => ['pending','away'].includes(j.journey.state) && j.journey.escort_hero_id === h.id))} onClose={() => setDispatchId('')}/> : null}
    {selectedJourney ? <EngineJourneySheet heroes={detail.roster.heroes} status={selectedJourney} prisoners={own} engineName={engines.data?.find(e => e.id === selectedJourney.journey.engine_id)?.name ?? 'Engine of Chaos'} canEdit={canEdit} gm={gm} onClose={() => setJourneyId('')} onReturn={() => {setJourneyId('');setReturnId(selectedJourney.journey.id)}}/> : null}
    {returnJourney ? <EngineReturnSheet status={returnJourney} roster={detail.roster} onClose={() => setReturnId('')}/> : null}
