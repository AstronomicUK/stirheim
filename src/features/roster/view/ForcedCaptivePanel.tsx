...
export function ForcedCaptivePanel({item,owner,captor,submitLabel}:{item:CaptiveCase;owner:WarbandDetail;captor:WarbandDetail;submitLabel:string}){
 ...
 return <ForcedCaptiveForm key={item.id} name={item.hero_name} allowWretch={captor.roster.warbandTemplateId==='court_of_the_profane_pleasures'} ownerGold={owner.warband.gold} pending={propose.isPending} submitLabel={submitLabel} error={error||propose.error?.message} onSubmit={choice=>{
  setError('')
  try{
