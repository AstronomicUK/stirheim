...
import { HIRED_SWORDS } from "./hiredSwords";
...
WARBAND_SKILL_TABLES.push({"id": "cavalry", "name": "Cavalry skills", "warbandId": "optional_cavalry", "intro": "Skills may only be used one at a time. Warriors without Ride may not use special riding skills.", "skills": [{"id": "cavalry_ride_horse", "name": "Ride Horse", "text": "This skill is vital if a rider wishes to ride an animal into combat. The skill is specific to a particular type of animal and must be gained again if the warrior wishes to be able to ride a different kind of animal. For instance, a warrior with Ride Horse would need to gain the skill Ride Warhorse if he wanted to be able to ride such a spirited mount."}, {"id": "cavalry_ride_warhorse", "name": "Ride Warhorse", "text": "This skill is vital if a rider wishes to ride an animal into combat. The skill is specific to a particular type of animal and must be gained again if the warrior wishes to be able to ride a different kind of animal. For instance, a warrior with Ride Horse would need to gain the skill Ride Warhorse if he wanted to be able to ride such a spirited mount."}, {"id": "cavalry_cavalry_commander", "name": "Cavalry Commander", "text": "Mounted Heroes are an impressive sight. With a good vantage point, they can see (and be seen) far more readily than if they were on foot. If the warband's leader has this skill and is mounted, he may add an extra 6\" to the distance within which other warriors in the warband may use his Leadership. This is in addition to any other bonuses that increase the range of the leader's influence.", "restriction": "Requires the appropriate Ride skill"}, {"id": "cavalry_trick_riding", "name": "Trick Riding", "text": "By athletically hanging off the side of his mount, a rider makes himself harder to hit. While a rider is trick riding all missile attacks against him suffer -1 to hit in addition to other modifiers. The rider must declare that he is trick riding before moving. He must then make an Initiative test and, if successful, may move full distance. If he fails he loses control of his mount and must roll immediately on the Whoa Boy! table. This skill may not be used with heavy armour because of the agility required. In addition, trick riding requires both hands, so the model may not use a shield or missile weapons whilst using the skill.", "restriction": "Requires the appropriate Ride skill"}, {"id": "cavalry_combat_riding", "name": "Combat Riding", "text": "The rider has trained his mount to use its bulk to trample any un­mounted enemy before him. A warrior with this skill may make a single additional S4 attack when charging an unmounted opponent. In subsequent rounds of combat, or if charged by enemy warriors, the mounted warrior fights as normal.", "restriction": "Requires the appropriate Ride skill"}, {"id": "cavalry_evade", "name": "Evade", "text": "The rider has trained his mount to swerve from side to side in combat, wrong­-footing his opponent. A rider with this skill always strikes first in close combat against dismounted opponents. When charged by an opponent, or otherwise fighting an enemy also entitled to strike first, attacks are carried out in order of Initiative. If Initiative is equal, the model with greater experience strikes first.", "restriction": "Requires the appropriate Ride skill"}, {"id": "cavalry_running_dismount", "name": "Running Dismount", "text": "The rider is able to dismount from his mount at speed. The rider may ride up to the mount's normal move distance and then dismount immediately. No further movement or shooting is possible. This skill may be used to move into contact with the enemy, counting as a Diving Charge from a height of 2\" - all the usual rules for diving charges apply. Note that the rider then counts as dismounted, gaining no further assistance from his mount.", "restriction": "Requires the appropriate Ride skill"}, {"id": "cavalry_athletic_mount", "name": "Athletic Mount", "text": "Without breaking stride, the warrior is able to leap onto the back of his mount and immediately spur it into a full gallop. Once the warrior is on board, the mount may make a run or charge move as normal. The warrior must be within half his full move distance of his steed to use this skill.", "restriction": "Requires the appropriate Ride skill"}, {"id": "cavalry_horse_archer", "name": "Horse Archer", "text": "The rider has learned the skills of the steppe nomads and can shoot from a running mount. The rider may shoot in a 360 degree arc whilst mounted, and may shoot while his mount is running; however the shot suffers a -1 to hit penalty in addition to all other normal modifiers.", "restriction": "Requires the appropriate Ride skill"}, {"id": "cavalry_mounted_combat_master", "name": "Mounted Combat Master", "text": "The rider is especially skilled at combat against a mounted opponent. If the model is fighting mounted against a mounted opponent and successfully wounds the enemy, the wounded model must add +1 to his roll on the Whoa Boy! table.", "restriction": "Requires the appropriate Ride skill"}], "source": {"publication": "Blazing Saddles", "file": "03-campaigns-magic-optional-rules.md:4462-4509"}});

// Preserve the source's complete hired-sword-only tables in the same searchable catalogue.
WARBAND_SKILL_TABLES.push(...HIRED_SWORDS.flatMap(entry => entry.detail?.uniqueSkills ? [{
  id: `hired_${entry.id}_skills`, name: entry.detail.uniqueSkills.tableName,
  warbandId: `hired_sword:${entry.id}`, intro: entry.detail.uniqueSkills.intro,
  skills: entry.detail.uniqueSkills.skills.map(skill => ({ ...skill, id: `hired_${entry.id}_skills_${skill.name.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '')}` })),
  source: { publication: entry.source, file: entry.detail.sourceFile },
}] : []));
