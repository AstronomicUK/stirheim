...
describe("applyHouseRuleDefaults", () => {
  it("returns the group defaults for nothing", () => {
    const expected = { strengthArmourPiercing: false, optionalCriticalTables: true, halfPriceArmour: true };
...
describe("describeHouseRules", () => {
  it("gives one line per switch reflecting the setting", () => {
    ...
    const off = describeHouseRules({ strengthArmourPiercing: true, optionalCriticalTables: false, halfPriceArmour: false });
