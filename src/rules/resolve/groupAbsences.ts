...
export function absentGroupModels(group:Pick<RosterHenchmanGroup,'size'|'campaignState'>):number {
 return Math.min(group.size,(group.campaignState?.reanimationOwed??0)+(group.campaignState?.constructRepairs?.length??0)+(group.campaignState?.raidAbsences??[]).filter(a=>a.games>0).reduce((n,a)=>n+a.count,0))
}
