...
export function ladyBlessingReason(weapon: Weapon, attackerWarbandId: string, targetWarbandId: string, targetUnitId: string | undefined, blessedWarbandIds: readonly string[]): string | undefined {
  ...
  if (BLACKPOWDER.has(weapon.id) || weapon.special.includes('blackpowderMisfireRulesAlwaysOn')) return 'Blessing of the Lady: roll 4+ before firing this blackpowder shot.'
  if (opponents.includes(targetWarbandId) && targetUnitId && KNIGHTS.has(targetUnitId)) return 'Blessing of the Lady: roll 4+ before shooting at this Questing Knight or Knight Errant.'
}
