...
export function recruitmentBlock(
  ...
  if (unit.id === leader?.id && warband.heroes.some(h => h.unitTemplateId === unit.id && h.status === 'dead') && !['the_undead','lizardmen','battle_monks_of_cathay','ogre_hunting_party','dreamwalkers'].includes(template.id)) return 'A slain leader cannot be bought again. Appoint an eligible surviving Hero instead.';
