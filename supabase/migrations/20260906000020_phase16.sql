-- Phase 16: kit consumed by a battle, and the campaign "bans" house rule.
--
-- 1. apply_battle_report reads `applied.item_patches` ([{id, quantity?, notes?}]): consumables
--    marked as used on the battle sheet are one fewer (a quantity of 0 removes the row), and a
--    Mordheim Map whose re-rolls were used is noted as spent. The before-values go in `undo` so
--    revert_battle_report can put them back.
-- 2. The campaigns.settings default gains houseRules.bans (id lists of items, spells, hired swords,
--    Dramatis Personae and skills removed from the campaign). Older rows parse without it: the
--    client's zod schema fills the default.
...
-- ---------------------------------------------------------------------------------------------
-- apply_battle_report: item patches
-- ---------------------------------------------------------------------------------------------
