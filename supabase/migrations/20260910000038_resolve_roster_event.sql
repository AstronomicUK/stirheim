-- Single-warband injury events and their earned advancement boxes commit together.
...
begin
 ...
 for a in select * from jsonb_array_elements(p_advances) loop
  ...
  where exists(select 1 from public.heroes h where h.id=(a->>'subject_id')::uuid and h.warband_id=p_warband_id and h.xp>=(a->>'threshold_xp')::int)
