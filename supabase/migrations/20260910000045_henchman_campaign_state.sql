-- Persist per-battle henchman upkeep, including report rollback.
alter table public.henchman_groups add column campaign_state jsonb not null default '{}'::jsonb check(jsonb_typeof(campaign_state)='object');
    ...
    if v_table = 'warbands' then
      ...
      if v_op = 'insert' then
        insert into public.henchman_groups (id, warband_id, name, unit_type_rules_id, size, stats, xp, level_ups, stat_increases,
                                            is_large, notes, sort_order, model_names, campaign_state)
        values (coalesce(v_id, gen_random_uuid()), p_warband_id, v_data ->> 'name', v_data ->> 'unit_type_rules_id', coalesce((v_data ->> 'size')::int, 1),
                ...
                coalesce(public.jsonb_text_array(v_data -> 'model_names'), '{}'::text[]), coalesce(v_data->'campaign_state','{}'::jsonb));
        get diagnostics v_rows = row_count;
      elsif v_op = 'update' then
        update public.henchman_groups set
          ...
          campaign_state = coalesce(v_data->'campaign_state',campaign_state)
        where id = v_id and warband_id = p_warband_id;
        get diagnostics v_rows = row_count;
      elsif v_op = 'delete' then
...
begin
  ...
  for v_row in select * from jsonb_array_elements(coalesce(v_applied -> 'groups', '[]'::jsonb)) loop
    v_patch := coalesce(v_row -> 'patch', '{}'::jsonb);
    select jsonb_build_object('id', g.id, 'before', jsonb_build_object('size', g.size, 'xp', g.xp, 'level_ups', g.level_ups, 'campaign_state', g.campaign_state))
