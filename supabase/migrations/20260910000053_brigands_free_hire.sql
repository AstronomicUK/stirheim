...
-- Validate the chosen still-standing outlaw and record the actual completed hire.
create or replace function public.validate_brigands_hire()
returns trigger language plpgsql security definer set search_path='' as $$
declare reward text; report public.match_reports%rowtype;
begin
  reward := new.flags->>'returningFavourReportId';
  if reward is null or reward not like '%:brigands' then return new; end if;
  if TG_OP='UPDATE' and old.flags->>'returningFavourReportId'=reward then return new; end if;
  select * into report from public.match_reports where id=split_part(reward,':',1)::uuid for update;
  if report.id is null or report.warband_id<>new.warband_id or report.status<>'applied'
     or not coalesce((report.applied->'scenario_free_hire'->'choices') ? new.hired_sword_rules_id,false) then
    raise exception 'This Brigands free hire is not available for this character';
  end if;
  update public.match_reports set notes=concat_ws(E'\n',notes,'Brigands free hire recruited: '||new.name||' ('||replace(new.hired_sword_rules_id,'_',' ')||'). Ordinary upkeep applies.') where id=report.id;
  return new;
end $$;
drop trigger if exists validate_brigands_hire on public.heroes;
create trigger validate_brigands_hire before insert or update of flags on public.heroes for each row execute function public.validate_brigands_hire();
