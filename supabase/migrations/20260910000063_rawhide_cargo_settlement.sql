-- Rawhide settlement runs inside report application, with shared treasury locks and checked reversal.
create or replace function public.apply_rawhide_settlement(p_report_id uuid)
returns jsonb language plpgsql volatile security definer set search_path='' as $$
declare r public.match_reports%rowtype; c public.rawhide_cargo%rowtype; outcome text; proceeds integer; merchant_delta integer; recipient_gold integer:=0; recipient_shards integer:=0;
begin
  select * into r from public.match_reports where id=p_report_id;
  if r.applied->'rawhide_settlement' is null then return null; end if;
  perform 1 from public.matches m where m.id=r.match_id and m.scenario_rules_id='rawhide' and m.state in ('awaiting_reports','completed') for update;
  if not found then raise exception 'End the Rawhide battle before revealing and settling its cargo'; end if;
  select * into c from public.rawhide_cargo where match_id=r.match_id for update;
  if c.match_id is null or c.reserved_at is null then raise exception 'Rawhide settlement requires the cargo declared before battle'; end if;
  if c.settled_report_id is not null then raise exception 'This Rawhide cargo has already been settled'; end if;
  if not exists(select 1 from public.match_participants where match_id=r.match_id and warband_id=r.warband_id) then raise exception 'Only a battle participant can settle cargo'; end if;
  outcome:=r.applied->'rawhide_settlement'->>'outcome';
  if outcome is null or outcome not in ('escaped','captured','empty') then raise exception 'Record whether the declared cargo escaped or was captured, or the wagons were empty'; end if;
  if (c.wagon is null) is distinct from (outcome='empty') then raise exception 'The outcome does not match the pre-battle cargo declaration'; end if;
  if (outcome in ('escaped','empty') and r.warband_id<>c.warband_id) or (outcome='captured' and r.warband_id=c.warband_id) then raise exception 'The merchant settles escaped or empty cargo; the capturing opponent settles captured cargo'; end if;
  perform 1 from public.warbands w where w.id in (c.warband_id,r.warband_id) order by w.id for update;
  if not exists(select 1 from public.warbands where id=c.warband_id and gold>=c.gold and wyrdstone>=c.wyrdstone) then raise exception 'Restore the declared merchant cargo before settlement'; end if;
  proceeds:=case when outcome='escaped' then case when c.rounding='up' then ceil(c.sale_value::numeric*13/10)::integer else floor(c.sale_value::numeric*13/10)::integer end else 0 end;
  merchant_delta:=proceeds-c.gold;
  if outcome='captured' then recipient_gold:=c.gold;recipient_shards:=c.wyrdstone; end if;
  update public.rawhide_cargo set settled_report_id=r.id where match_id=r.match_id;
  update public.warbands set gold=gold+merchant_delta,wyrdstone=wyrdstone-c.wyrdstone where id=c.warband_id;
  if outcome='captured' then update public.warbands set gold=gold+recipient_gold,wyrdstone=wyrdstone+recipient_shards where id=r.warband_id; end if;
  return jsonb_build_object('merchant_id',c.warband_id,'merchant_gold_delta',merchant_delta,'merchant_shards_delta',-c.wyrdstone,'recipient_gold_delta',recipient_gold,'recipient_shards_delta',recipient_shards,'outcome',outcome);
end; $$;
revoke all on function public.apply_rawhide_settlement(uuid) from public,authenticated;

create or replace function public.revert_rawhide_settlement(p_report_id uuid)
returns void language plpgsql volatile security definer set search_path='' as $$
declare r public.match_reports%rowtype; u jsonb; snapshot jsonb;
begin
  select * into r from public.match_reports where id=p_report_id;
  u:=r.undo->'rawhide_cargo';
  if u is null or u='null'::jsonb then return; end if;
  perform 1 from public.matches where id=r.match_id for update;
  perform 1 from public.rawhide_cargo where match_id=r.match_id and settled_report_id=r.id for update;
  if not found then raise exception 'The Rawhide settlement record changed; reconcile it before withdrawal'; end if;
  perform 1 from public.warbands w where w.id in (r.warband_id,(u->>'merchant_id')::uuid) order by w.id for update;
  for snapshot in select * from jsonb_array_elements(u->'after') loop
    if not exists(select 1 from public.warbands w where w.id=(snapshot->>'id')::uuid and w.gold=(snapshot->>'gold')::integer and w.wyrdstone=(snapshot->>'wyrdstone')::integer) then raise exception 'A treasury changed after Rawhide settlement; reverse later spending or rewards before withdrawal'; end if;
  end loop;
  -- The filing warband's cargo and ordinary report deltas are reversed together below,
  -- allowing proceeds spent within this same report without a transient negative balance.
  if (u->>'merchant_id')::uuid<>r.warband_id then
    update public.warbands set gold=gold-(u->>'merchant_gold_delta')::integer,wyrdstone=wyrdstone-(u->>'merchant_shards_delta')::integer where id=(u->>'merchant_id')::uuid;
  end if;
end; $$;
revoke all on function public.revert_rawhide_settlement(uuid) from public,authenticated;
...
begin
  ...
  perform public.revert_rawhide_settlement(v_report.id);
  ...
  if v_undo->'rawhide_cargo' is not null and v_undo->'rawhide_cargo'<>'null'::jsonb then
    v_row:=v_undo->'rawhide_cargo';
    v_wb:=v_wb || jsonb_build_object(
      'gold_delta',coalesce((v_wb->>'gold_delta')::int,0)+case when (v_row->>'merchant_id')::uuid=v_report.warband_id then (v_row->>'merchant_gold_delta')::int else (v_row->>'recipient_gold_delta')::int end,
      'wyrdstone_delta',coalesce((v_wb->>'wyrdstone_delta')::int,0)+case when (v_row->>'merchant_id')::uuid=v_report.warband_id then (v_row->>'merchant_shards_delta')::int else (v_row->>'recipient_shards_delta')::int end);
  end if;
  update public.warbands set
    wyrdstone = greatest(0, wyrdstone - coalesce((v_wb ->> 'wyrdstone_delta')::int, 0)),
    gold = greatest(0, gold - coalesce((v_wb ->> 'gold_delta')::int, 0)),
  ...
  if v_undo->'rawhide_cargo' is not null and v_undo->'rawhide_cargo'<>'null'::jsonb then
    update public.rawhide_cargo set settled_report_id=null where match_id=v_report.match_id and settled_report_id=v_report.id;
    -- Recheck all reservations after restoring the original cargo.
    update public.warbands set gold=gold where id=(v_undo->'rawhide_cargo'->>'merchant_id')::uuid;
  end if;
