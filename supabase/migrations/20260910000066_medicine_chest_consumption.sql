-- Consume exactly the reviewed Medicine Chest copies; a stale report cannot reuse the same stock.
create or replace function public.validate_medicine_chests(p_report_id uuid)
returns void language plpgsql security definer set search_path='' as $$
declare r public.match_reports%rowtype; u jsonb; i public.items%rowtype; n integer;
begin
 select * into r from public.match_reports where id=p_report_id;
 if r.applied->'medicine_chests' is null then return; end if;
 if jsonb_typeof(r.applied->'medicine_chests')<>'array' then raise exception 'Review the Medicine Chest use record'; end if;
 if exists(select 1 from jsonb_array_elements(r.applied->'medicine_chests') x group by x->>'item_id' having count(*)>1) then raise exception 'Record each Medicine Chest stack once'; end if;
 perform 1 from public.items it where it.id in (select (x->>'item_id')::uuid from jsonb_array_elements(r.applied->'medicine_chests') x) order by it.id for update;
 for u in select * from jsonb_array_elements(r.applied->'medicine_chests') loop
   select * into i from public.items where id=(u->>'item_id')::uuid;
   n:=(u->>'quantity')::integer;
   if i.id is null or i.warband_id<>r.warband_id or i.item_rules_id is distinct from 'scenario_medicine_chest' or n is null or n<1 or n>i.quantity or (u->>'expected_quantity')::integer is distinct from i.quantity then raise exception 'The Medicine Chest stock changed; review its available copies before filing'; end if;
   if (select count(*) from jsonb_array_elements(coalesce(r.applied->'item_patches','[]'::jsonb)) p where p->>'id'=i.id::text)<>1 or not exists(select 1 from jsonb_array_elements(coalesce(r.applied->'item_patches','[]'::jsonb)) p where p->>'id'=i.id::text and (p->>'quantity')::integer=i.quantity-n) then raise exception 'The injury reroll must consume exactly the recorded Medicine Chest copies'; end if;
 end loop;
end; $$;
revoke all on function public.validate_medicine_chests(uuid) from public,authenticated;
