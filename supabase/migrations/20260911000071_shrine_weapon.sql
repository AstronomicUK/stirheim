-- Preserve the identity and quantity of the one weapon selected for the Shrine blessing.
create or replace function public.validate_shrine_equipment(p_report_id uuid)
returns void language plpgsql security definer set search_path='' as $$
declare r public.match_reports%rowtype; entry jsonb; i public.items%rowtype;
begin
 select * into r from public.match_reports where id=p_report_id;
 entry:=r.applied->'shrine_equipment';
 if entry is null then return; end if;
 if r.exploration->>'locationId' is distinct from 'shrine' or not exists(select 1 from public.warbands w where w.id=r.warband_id and w.type_rules_id in ('witch_hunters','sisters_of_sigmar')) then raise exception 'This warband has no Shrine weapon blessing to apply'; end if;
 select * into i from public.items where id=(entry->>'item_id')::uuid and warband_id=r.warband_id for update;
 if i.id is null or i.quantity<1 or to_jsonb(i)-'created_at'-'updated_at' is distinct from entry->'expected' then raise exception 'The Shrine weapon changed; review the remaining weapon before filing'; end if;
end $$;
revoke all on function public.validate_shrine_equipment(uuid) from public,authenticated;
...
begin
  ...
  perform public.validate_shrine_equipment(v_report.id);
