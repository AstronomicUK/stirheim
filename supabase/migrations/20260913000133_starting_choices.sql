-- Persist all starting choices and construct controllers in the creation transaction.
...
begin
  ...
  for v_hero in select * from jsonb_array_elements(coalesce(payload -> 'heroes', '[]'::jsonb)) loop
    insert into public.heroes (warband_id, name, unit_type_rules_id, stats, xp, level_ups, skill_tables, is_large, sort_order, flags, skills, spells)
    values (
      v_warband_id,
      v_hero ->> 'name',
      ...
      coalesce((select array_agg(x) from jsonb_array_elements_text(coalesce(v_hero -> 'skills', '[]'::jsonb)) as x), '{}'),
      coalesce((select array_agg(x) from jsonb_array_elements_text(coalesce(v_hero -> 'spells', '[]'::jsonb)) as x), '{}')
    )
  ...
  for v_group in select * from jsonb_array_elements(coalesce(payload -> 'henchman_groups', '[]'::jsonb)) loop
    insert into public.henchman_groups (warband_id, name, unit_type_rules_id, size, stats, xp, level_ups, is_large, sort_order, campaign_state)
    values (
      v_warband_id,
      v_group ->> 'name',
      ...
      coalesce(v_group -> 'campaign_state', '{}'::jsonb)
    )
    returning id into v_holder_id;
